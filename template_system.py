...
async def _run_wings_panel_connect(
    ...
    await _upd(2)
    ok, out = await _exec(
        # Remove stale pterodactyl_nw network so Wings can recreate it with the
        # correct 172.21.0.0/16 subnet (avoids "Pool overlaps" Docker error).
...
async def _run_wings_installation(
    ...
    if panel_connected:
        if not await _step(
            8,
            # Remove any stale pterodactyl_nw network (wrong subnet from a prior run)
            # so Wings can create it fresh with the configured 172.21.0.0/16 subnet.
            "docker network rm pterodactyl_nw 2>/dev/null || true; "
