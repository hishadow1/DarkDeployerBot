...
def _embed_panel_connect(container: str, logo: str) -> discord.Embed:
    e = discord.Embed(
        title="🔌  Connect Wings to Your Panel?",
        description=(
            "DarkNodes can **automatically** create a location and node in your "
            "Pterodactyl Panel, then write the real Wings configuration so the daemon "
            "starts connected and ready.\n\n"
            "**You will need:**\n"
            "• Your Panel URL — e.g. `https://panel.example.com`\n"
            "• An **Admin API Key** from your Panel "
            "*(Admin → Application API → Create)*\n\n"
            "If you'd rather configure Wings manually, click **Skip**."
        ),
        color=0x5865F2,
    )
...
class _WingsPanelConnectView(discord.ui.View):
    """
    Shown after SSL selection.  Offers two paths:
      • 🔌 Connect to Panel — opens a modal to collect Panel URL + API key,
        then the installer will auto-create a location + node and write the
        real config.yml before starting Wings.
      • ⏭ Skip             — proceeds with a placeholder config; the user
        configures Wings manually from the Panel afterward.
    """
    ...
    @discord.ui.button(label="⏭ Skip — Manual Setup", style=discord.ButtonStyle.secondary, row=0)
...
async def _run_wings_installation(
    ...
    # 7 — Connect to Panel (auto-create location + node, write real config)
    ...
    if panel_url and panel_api_key:
        ...
        try:
            ...
            status, body = await _panel_api(
                ...
                {"short": "darknodes", "long": f"DarkNodes ({container})"},
            )
            if status not in (200, 201):
                err_text = body if isinstance(body, str) else str(body)
                await _upd(7, failed_at=7, error=f"Failed to create Panel location (HTTP {status}):\n{err_text[:300]}")
                return
            ...
            if status not in (200, 201):
                err_text = body if isinstance(body, str) else str(body)
                await _upd(7, failed_at=7, error=f"Failed to create Panel node (HTTP {status}):\n{err_text[:300]}")
                return
    ...
    # 8 — Start Wings (with --debug via the service file written in step 6)
    await _upd(8)
    await _exec("systemctl start wings 2>&1; sleep 3; true", timeout=30)
    await _upd(9)

    # 9 — Verify binary is executable (wings has no --version flag; just check the file)
    ok, out = await _exec(
        "test -x /usr/local/bin/wings && echo wings_ok || echo wings_missing",
        timeout=10,
    )
    if not ok or "wings_ok" not in out:
        await _upd(9, failed_at=9, error=f"Wings binary not found or not executable:\n{out[:300]}")
        return
    ...
    if panel_connected:
        ...
        desc = (
            ...
            f"🌐 **Wings address:** `{scheme}://{domain}:8080`\n\n"
            f"> 📬 Manual setup steps have been sent to your DMs."
        )
    ...
    try:
        if panel_connected:
            ...
            dm = discord.Embed(
                ...
                description=(
                    "Wings is installed and running in debug mode.\n"
                    "You still need to connect it to your Pterodactyl Panel:\n\n"
            ...
            dm.add_field(
                ...
                value=(
                    ...
                    "5. `systemctl restart wings`\n"
                    "6. Node turns **green** in your Panel ✅"
                ),
                inline=False,
            )
            dm.add_field(
