...
def _safe_hostname(value: str) -> bool:
    """Accept only a DNS hostname before it reaches shell/API configuration."""
...
def _safe_absolute_path(value: str) -> bool:
    """Certificate paths are later embedded in shell/YAML commands."""
    return bool(re.fullmatch(r"/[A-Za-z0-9._/-]+", (value or "").strip()))
...
async def _run_wings_installation(
    ...
    if not _safe_hostname(domain):
        await _upd(0, failed_at=0, error="Invalid Wings hostname. Use a DNS name such as wings.example.com.")
        return
    if ssl_mode == "custom" and (
        not _safe_absolute_path(cert_path) or not _safe_absolute_path(key_path)
    ):
        await _upd(0, failed_at=0, error="Certificate paths must be absolute local paths without shell characters.")
        return
    ...
    if not await _step(1, "\n".join([
        "DEBIAN_FRONTEND=noninteractive apt-get install -y -qq curl wget tar file 2>&1 | tail -5",
    ...
    arch_cmd = "\n".join([
        ...
        # Confirm the file exists and is an ELF executable, not an HTML error page.
        'SIZE=$(stat -c%s /usr/local/bin/wings 2>/dev/null || echo 0)',
        'FILE_TYPE=$(file -b /usr/local/bin/wings 2>/dev/null || true)',
        'if [ "$SIZE" -gt 1000000 ] && printf "%s" "$FILE_TYPE" | grep -qi "ELF"; then '
        'echo "wings_downloaded OK size=${SIZE} type=${FILE_TYPE}"; '
        'else echo "wings_download_failed size=${SIZE} type=${FILE_TYPE}"; exit 1; fi',
    ])
    ...
    wings_svc = "\n".join([
        ...
        "LimitNOFILE=4096",
        "ExecStart=/usr/local/bin/wings --config /etc/pterodactyl/config.yml --debug",
        "Restart=on-failure",
        "StartLimitInterval=180",
        "StartLimitBurst=30",
        "RestartSec=5s",
    ...
    if panel_connected:
        if not await _step(
            8,
            # Poll until active or timeout (~20s). Wings can take several seconds
    ...
    health_scheme = "https" if ssl_mode != "http" else "http"
    ok, out = await _exec(
        "test -x /usr/local/bin/wings && "
        "(systemctl is-active --quiet wings && "
        " ss -ltn '( sport = :8080 )' | grep -q LISTEN && "
        f" curl --silent --show-error --insecure --max-time 5 "
        f" {health_scheme}://127.0.0.1:8080/ -o /dev/null -w '%{{http_code}}' "
        " | grep -qE '^[1-5][0-9][0-9]$' && "
        " echo wings_ok || echo wings_config_pending)",
        timeout=10,
    )
