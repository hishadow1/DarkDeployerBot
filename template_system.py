...
async def _run_wings_panel_connect(
    ...
    await _upd(2)
    ok, out = await _exec(
        # Force-remove any Docker network overlapping 172.21.0.0/16 before starting Wings.
        ...
        r'echo "$subnet" | grep -qE "172\.21\." || continue; '
...
async def _run_wings_installation(
    ...
    # ExecStartPre script: remove pterodactyl_nw and any other Docker network
    # whose subnet overlaps 172.21.0.0/16 before Wings tries to create it.
    # docker network prune won't touch networks that still have containers
    # attached, so we force-disconnect first.
    _wings_net_cleanup = (
        r"docker network rm pterodactyl_nw 2>/dev/null || true; "
        r"for net in $(docker network ls -q 2>/dev/null); do "
        r'subnet=$(docker network inspect "$net" --format '
        r'"{{range .IPAM.Config}}{{.Subnet}} {{end}}" 2>/dev/null); '
        r'echo "$subnet" | grep -qE "172\.21\." || continue; '
        r'for cid in $(docker network inspect "$net" --format '
        r'"{{range $k,$v := .Containers}}{{$v.Name}} {{end}}" 2>/dev/null); do '
        r'docker network disconnect -f "$net" "$cid" 2>/dev/null || true; done; '
        r'docker network rm "$net" 2>/dev/null || true; done; '
        # Wings creates the pterodactyl0 bridge interface itself.  If a previous
        # Wings run crashed mid-startup the interface can persist with a 172.21.x.x
        # address assigned, causing Docker to refuse the new network ("Pool overlaps").
        # Bring it down and delete it so the subnet is fully released.
        r"ip link set pterodactyl0 down 2>/dev/null || true; "
        r"ip link delete pterodactyl0 2>/dev/null || true"
    )
    wings_svc = "\n".join([
        ...
        "LimitNOFILE=65535",
        f"ExecStartPre=/bin/bash -c '{_wings_net_cleanup}'",
    ...
    if panel_connected:
        if not await _step(
            8,
            # Force-remove any Docker network overlapping 172.21.0.0/16.
            ...
            # the 172.21.x.x subnet even after the network object is removed.
            r"docker network rm pterodactyl_nw 2>/dev/null || true; "
            r"for net in $(docker network ls -q 2>/dev/null); do "
            r'subnet=$(docker network inspect "$net" --format "{{range .IPAM.Config}}{{.Subnet}} {{end}}" 2>/dev/null); '
            r'echo "$subnet" | grep -qE "172\.21\." || continue; '
