...
async def _run_wings_panel_connect(
    ...
    await _upd(2)
    ok, out = await _exec(
        # Remove stale pterodactyl_nw network and prune all unused Docker networks
        # so Wings can recreate it with the correct subnet (avoids "Pool overlaps" error).
...
async def _run_wings_installation(
    ...
    if not await _step(1, "\n".join([
        ...
        "for i in $(seq 1 10); do systemctl is-active --quiet docker && break; sleep 1; done",
    ...
    if panel_connected:
        if not await _step(
            8,
            # Remove any stale pterodactyl_nw network and prune all unused Docker
            # networks (avoids "Pool overlaps" error from other leftover subnets).
            "docker network rm pterodactyl_nw 2>/dev/null || true; "
            "docker network prune -f 2>/dev/null || true; "
