...
async def _pick_free_wings_subnet(exec_fn) -> str:
    ...
    Queries existing Docker network subnets and the kernel routing table, then
...
async def _run_wings_panel_connect(
    ...
    ok, out = await _exec(
        ...
        r"ip link set pterodactyl0 down 2>/dev/null || true; "
        r"ip link delete pterodactyl0 2>/dev/null || true; "
...
async def _run_wings_installation(
    ...
    wings_pre_start_sh = "\n".join([
        ...
        "# Removes Docker networks whose subnet overlaps the one Wings is configured",
        "# to use, and deletes any leftover pterodactyl0 bridge from a crashed run.",
        ...
        "",
        "# 2. Determine the Wings network prefix from its config (fallback: 172.21).",
        ...
        "",
        "# 3. Scan all networks for any subnet matching the prefix and force-remove them.",
        ...
        "done",
        "",
        "# 4. Delete the pterodactyl0 Linux bridge interface if it survived a",
        "#    previous crashed Wings run.  Docker will refuse a new network if",
        "#    this interface still holds the subnet.",
    ...
    if panel_connected:
        if not await _step(
            ...
            # Also delete the pterodactyl0 bridge interface left by a crashed
            # Wings run — Docker will refuse a new network if it still holds
            # the subnet even after the network object is removed.
            r"docker network rm pterodactyl_nw 2>/dev/null || true; "
            r"for net in $(docker network ls -q 2>/dev/null); do "
            r'subnet=$(docker network inspect "$net" --format "{{range .IPAM.Config}}{{.Subnet}} {{end}}" 2>/dev/null); '
            f'echo "$subnet" | grep -qF "{_wings_prefix}." || continue; '
