...
async def _run_wings_installation(
    ...
    if panel_url and panel_api_key:
        ...
        try:
            ...
            # We never write the Panel YAML verbatim because it sets api.port to
            # daemon_listen (443 for Cloudflare mode) and uses cert paths that
    ...
    # 1) Edit the ephemeral progress message so the user sees the final result.
    try:
        await msg.edit(embed=done, view=success_view)
    except Exception:
        pass
    # 2) Also post a permanent (non-ephemeral) message in the channel so the
    #    "Configure Wings automatically" button persists and anyone can see it.
    try:
