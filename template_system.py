...
async def _run_wings_panel_connect(
    ...
    # Wings was installed with localhost-ssl (self-signed cert via Cloudflare Tunnel)
    ssl_enabled = "true"
...
async def _run_wings_installation(
    ...
    if not await _step(1, "\n".join([
        ...
        "ss -tlnp 2>/dev/null | grep -q ':2022 ' && { echo 'ERROR: Port 2022 is already in use. Wings SFTP cannot bind. Free that port and retry.'; exit 1; } || true",
        "ss -tlnp 2>/dev/null | grep -q ':8080 ' && { echo 'ERROR: Port 8080 is already in use. Wings API cannot bind. Free that port and retry.'; exit 1; } || true",
    ]), timeout=120): return
    ...
    arch_cmd = "\n".join([
        ...
        # Verify the binary is a real ELF executable (>10 MB) and can at least
        # be executed (wings exits non-zero without a config but must not SIGILL).
        'SIZE=$(stat -c%s /usr/local/bin/wings 2>/dev/null || echo 0)',
        'if [ "$SIZE" -lt 10000000 ]; then echo "wings_download_failed size=${SIZE}"; exit 1; fi',
        'file /usr/local/bin/wings | grep -q ELF || { echo "ERROR: Downloaded file is not a valid ELF binary (HTML error page?)"; exit 1; }',
