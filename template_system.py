...
# Shows the step-by-step Cloudflare guide, then one modal collects the 4
# required values and the bot does everything else automatically.
# ─────────────────────────────────────────────────────────────────────────────
class _WingsSetupGuideView(discord.ui.View):
    """
    Shown alongside the step-by-step Wings setup guide.
    'I'm Ready' opens the automated 4-field modal.
...
class _WingsAutomatedModal(discord.ui.Modal, title="🪽  Wings — Automated Setup"):
    """
    Collects the 4 values needed for full automation:
    Wings hostname, tunnel token, Panel URL, Panel API key.
    The bot then installs cloudflared → Wings → auto-connects to Panel.
    """

    wings_hostname = discord.ui.TextInput(
        label="Wings Hostname  (step 4 of guide)",
        placeholder="wings.example.com  (no https://)",
        min_length=4,
        max_length=253,
    )
    tunnel_token = discord.ui.TextInput(
        label="Cloudflare Tunnel Token  (step 3 of guide)",
        placeholder="eyJhIjoiN…  (the long string from the Cloudflare Install connector page)",
        required=True,
        min_length=20,
        max_length=2000,
    ...
    panel_api_key = discord.ui.TextInput(
        label="Panel Admin API Key  (step 5 of guide)",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Derive a clean tunnel name from the hostname's first label
        tunnel_name = wings_domain.split(".")[0] or "wings"

        embed = _progress_embed("🚀  Installing Wings via Cloudflare Tunnel", CLOUDFLARE_STEPS, 0, logo)
        ...
        asyncio.create_task(_run_cloudflare_installation(
            ...
            self.panel_api_key.value.strip(),
        ))
...
class _CloudflareTunnelModal(discord.ui.Modal, title="☁️  Cloudflare Tunnel Token"):
    ...
    wings_domain = discord.ui.TextInput(
    ...
    panel_api_key = discord.ui.TextInput(
        label="Panel Admin API Key",
        placeholder="ptla_xxxxxxxxxxxxxxxxxxxxxxxxx",
        min_length=10, max_length=500,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        node_id = self._vps_info.get("node_id")
        wings_domain = (
            self.wings_domain.value.strip().lower()
            .removeprefix("https://")
            .removeprefix("http://")
            .rstrip("/")
        )
...
class _CloudflareWingsSetupView(discord.ui.View):
    """Starts the fully automated HTTP Wings + Panel connection flow."""
...
async def _run_cloudflare_installation(
    ...
    await _upd(len(steps))
        ...
        description=(
            ...
            "Starting the automated Wings connection now. The bot will generate a "
            "self-signed SSL cert, install Wings with HTTPS, create or reuse the "
            "Panel node, write its configuration, and start Wings."
        ),
        color=0x57F287,
    )
    _set_footer(done, logo)
    ...
    if not panel_url or not panel_api_key or not wings_domain:
    ...
    asyncio.create_task(_run_wings_installation(
        ...
        container,
        wings_domain,
        "localhost-ssl",   # self-signed cert via hopingboyz/localhost-ssl method
        "",
        "",
        msg,
        logo,
...
def _cf_guide_wings_embed(logo: str) -> discord.Embed:
    ...
    e.add_field(
        ...
        value=(
            ...
            "Enter the Wings hostname, tunnel token, Panel URL, and Panel Admin API key."
        ),
        inline=False,
    )
    e.add_field(
