...
# ─────────────────────────────────────────────────────────────────────────────
def _make_exec(container: str):
    """Return an async exec(cmd, timeout) function bound to a container."""
...
class _PanelHTTPSView(_HTTPSView):
    ...
    async def _on_no(self, interaction: discord.Interaction):
        ...
        asyncio.create_task(_run_panel_installation(
            interaction.user, self._container, self._domain, "http", "", "", msg, logo
        ))
...
class _PanelSSLMethodView(_SSLMethodView):
    ...
    async def _on_letsencrypt(self, interaction: discord.Interaction):
        ...
        msg   = await _start_progress(interaction, embed)
        asyncio.create_task(_run_panel_installation(
            interaction.user, self._container, self._domain, "letsencrypt", "", "", msg, logo
        ))
...
# ─────────────────────────────────────────────────────────────────────────────
# Cloudflare DM Guide — comprehensive routing guide for Panel + Wings
# ─────────────────────────────────────────────────────────────────────────────
async def _send_cloudflare_guide_dm(
    ...
    try:
        ...
        overview = discord.Embed(
            title="☁️  Cloudflare Tunnel — Routing Guide",
            description=(
                f"Tunnel **`{tunnel_name}`** on `{container}` is live.\n\n"
                "Use this guide to route your Pterodactyl services through Cloudflare Zero Trust.\n"
                "You'll configure **public hostnames** in the Cloudflare dashboard — "
                "no firewall ports need to be opened."
            ),
            color=0xF6821F,
        )
        detect_lines = [
            (f"✅  Pterodactyl Panel detected — {panel_service} on port {panel_port}"
             if panel_detected else
             "⬜  Pterodactyl Panel — not detected (install it first if needed)"),
            (f"✅  Pterodactyl Wings detected — {wings_service} on port {wings_port}"
             if wings_detected else
             "⬜  Pterodactyl Wings — not detected (install it first if needed)"),
        ]
        overview.add_field(
            name="🔍  Auto-Detected Services",
            value="\n".join(detect_lines),
            inline=False,
        )
        overview.add_field(
        ...
        # ── Panel routing guide ───────────────────────────────────────────
        panel_scheme = "https" if panel_service == "HTTPS" else "http"
        panel_embed  = discord.Embed(
            title="🦖  Routing Pterodactyl Panel through Cloudflare",
            color=0x000000,
        )
        if panel_detected:
            panel_embed.description = f"✅  Auto-detected: **{panel_service}** on port **{panel_port}**"
        else:
            panel_embed.description = "ℹ️  Generic guide — adjust the service/port if your Panel is installed."

        panel_embed.add_field(
            name="1️⃣  Add a Public Hostname",
            value=(
                "In Cloudflare Zero Trust dashboard:\n"
                f"Tunnels → **`{tunnel_name}`** → Public Hostnames → **Add a Public Hostname**"
            ),
            inline=False,
        )
        panel_embed.add_field(
            ...
            value=(
                "```\n"
                f"Subdomain  : panel\n"
                f"Domain     : yourdomain.com\n"
                f"Service    : {panel_service}\n"
                f"URL        : 127.0.0.1:{panel_port}\n"
                "```"
            ),
            inline=False,
        )
        panel_embed.add_field(
            ...
            value=(
                "If your Panel uses **HTTPS** (port 443):\n"
                "→ Expand **Additional application settings**\n"
                "→ **TLS** → **No TLS Verify** → **Enabled**\n\n"
                "*(Cloudflare handles the public TLS; this disables verification of the local cert)*"
            ),
            inline=False,
        )
        panel_embed.add_field(
            name="4️⃣  SSL / TLS Mode (Cloudflare Dashboard → SSL/TLS)",
            value=(
                "**Full** → if your Panel uses HTTP internally\n"
                "**Full (Strict)** → if your Panel uses HTTPS with a valid cert\n"
                "**Flexible** → not recommended (insecure)"
            ),
            inline=False,
        )
        panel_embed.add_field(
            name="5️⃣  DNS Configuration",
            value=(
                "Cloudflare automatically creates a CNAME record when you add the hostname.\n"
                "Ensure the record is **Proxied** (orange cloud ☁️), not DNS-only."
            ),
            inline=False,
        )
        panel_embed.add_field(
            name="6️⃣  Security Recommendations",
            value=(
                "• Enable **Bot Fight Mode** in Security settings\n"
                "• Set **Security Level** to Medium or High\n"
                "• Enable **Under Attack Mode** if you see unusual traffic\n"
                "• Add a **Firewall Rule** to block non-Cloudflare IPs from reaching port 80/443 on the VPS"
            ),
            inline=False,
        )
        panel_embed.add_field(
            name="✅  Result",
            value=f"Your Panel will be accessible at: **`https://panel.yourdomain.com`**",
            inline=False,
        )
        ...
        wings_embed = discord.Embed(
            title="🪽  Routing Pterodactyl Wings through Cloudflare",
            color=0x000000,
        )
        if wings_detected:
            wings_embed.description = f"✅  Auto-detected: **{wings_service}** on port **{wings_port}**"
        else:
            wings_embed.description = "ℹ️  Generic guide — adjust the service/port if your Wings is installed."

        wings_embed.add_field(
            name="⚠️  Important Note about Wings + Cloudflare",
            value=(
                "Wings uses **WebSockets** for real-time communication with the Panel.\n"
                "Cloudflare supports WebSockets on **Pro plans and above**, or with HTTP/2 grpc.\n\n"
                "**If you are on the Free plan**, you can still route Wings through Cloudflare, "
                "but you may need to use a **direct IP** or a **separate A record** for the Wings FQDN "
                "set to **DNS-only** (grey cloud) to avoid WebSocket issues."
            ),
            inline=False,
        )
        wings_embed.add_field(
        ...
        wings_embed.add_field(
            ...
            value=(
                ...
                f"Subdomain  : wings\n"
                f"Domain     : yourdomain.com\n"
                f"Service    : {wings_service}\n"
                f"URL        : 127.0.0.1:{wings_port}\n"
                "```"
            ),
            inline=False,
        )
        wings_embed.add_field(
            ...
            value=(
                "→ **TLS** → **No TLS Verify** → **Enabled**\n"
                "→ **HTTP2 Connection** → **Enabled** (helps with WebSocket performance)\n\n"
                "Wings always uses HTTPS internally; disabling TLS verify is required "
                "so Cloudflare can forward to the local HTTPS listener."
            ),
            inline=False,
        )
        wings_embed.add_field(
            name="4️⃣  SSL / TLS Mode",
            value=(
                "Set to **Full** in the Cloudflare dashboard (SSL/TLS section).\n"
                "Do **not** use Flexible — Wings requires HTTPS end-to-end."
            ),
            inline=False,
        )
        wings_embed.add_field(
            name="5️⃣  Panel Node Configuration",
            value=(
                "In your Pterodactyl Panel → Admin → Nodes → (your Wings node):\n"
                f"• **FQDN**: `wings.yourdomain.com`\n"
                "• **Scheme**: `https`\n"
                "• **Port**: `443` *(Cloudflare terminates at 443)*\n\n"
                "Paste the updated node config to `/etc/pterodactyl/config.yml` on the Wings VPS, "
                "then run:\n```bash\nsystemctl restart wings\n```"
            ),
            inline=False,
        )
        wings_embed.add_field(
            name="6️⃣  DNS Configuration",
            value=(
                "If routing through the tunnel (Proxied): the CNAME is created automatically.\n\n"
                "**Alternative (DNS-only for WebSocket compatibility):**\n"
                "Create an **A record**: `wings.yourdomain.com → <your VPS IP>` with **grey cloud** (DNS-only)\n"
                "Then open port **8080** on your VPS firewall for Wings."
            ),
            inline=False,
        )
        wings_embed.add_field(
            name="✅  Result",
            value="Wings will be accessible at: **`https://wings.yourdomain.com`** (or `:8080` for DNS-only).",
            inline=False,
        )
...
# ─────────────────────────────────────────────────────────────────────────────
# /cloudflare  —  standalone interactive guide (no VPS / no install required)
# ─────────────────────────────────────────────────────────────────────────────
def _cf_guide_main_embed(logo: str) -> discord.Embed:
    ...
    e = discord.Embed(
        title="☁️  Cloudflare Tunnel — Routing Guide",
        description=(
            "Step-by-step instructions for routing your Pterodactyl services "
            "through Cloudflare Zero Trust.\n\n"
            "Select a service below to see its full configuration guide."
        ),
        color=0xF6821F,
    )
    e.add_field(
    ...
    e.add_field(
        ...
        value="HTTP/HTTPS routing, TLS settings, SSL mode, DNS, and security recommendations.",
        inline=True,
    )
    e.add_field(
        name="🪽  Pterodactyl Wings",
        value="HTTPS routing, WebSocket notes, Panel node config, DNS options.",
        inline=True,
    )
    _set_footer(e, logo)
    return e
...
def _cf_guide_panel_embed(logo: str) -> discord.Embed:
    ...
    e.add_field(
        ...
        value=(
            ...
            "Service    : HTTP  (or HTTPS if Panel uses SSL)\n"
            "URL        : 127.0.0.1:80  (or :443 for HTTPS)\n"
            "```"
        ),
        inline=False,
    )
    e.add_field(
        name="3️⃣  Advanced Settings (HTTPS Panels only)",
        value=(
            "If your Panel uses HTTPS internally:\n"
            "→ Expand **Additional application settings**\n"
            "→ **TLS** → **No TLS Verify** → **Enabled**\n\n"
            "*(Cloudflare provides the public TLS; this disables verification of the local cert.)*"
        ),
        inline=False,
    )
    e.add_field(
        name="4️⃣  SSL / TLS Mode  (Cloudflare → SSL/TLS)",
        value=(
            "**Full** — Panel uses HTTP internally\n"
            "**Full (Strict)** — Panel uses HTTPS with a valid cert\n"
            "**Flexible** — not recommended (insecure)"
        ),
        inline=False,
    )
    e.add_field(
        name="5️⃣  DNS",
        value=(
            "Cloudflare creates a CNAME automatically when you add the hostname.\n"
            "Make sure it is **Proxied** (orange cloud ☁️), not DNS-only."
        ),
        inline=False,
    )
    e.add_field(
        ...
        value=(
            ...
            "• Set **Security Level** to Medium or High\n"
            "• Block non-Cloudflare IPs on ports 80/443 via your VPS firewall"
        ),
        inline=False,
    )
    e.add_field(
        name="✅  Result",
        value="Panel accessible at: **`https://panel.yourdomain.com`**",
        inline=False,
    )
    _set_footer(e, logo)
    return e
...
def _cf_guide_wings_embed(logo: str) -> discord.Embed:
    ...
    e.add_field(
        name="⚠️  WebSocket Note",
        value=(
            "Wings uses **WebSockets** for real-time communication.\n"
            "Cloudflare supports WebSockets on **Pro plans and above**.\n\n"
            "On the **Free plan**: use a DNS-only A record for Wings "
            "(grey cloud) and open port **8080** on your VPS firewall."
        ),
        inline=False,
    )
    e.add_field(
    ...
    e.add_field(
        ...
        value=(
            "→ **TLS** → **No TLS Verify** → **Enabled**\n"
            "→ **HTTP2 Connection** → **Enabled** *(improves WebSocket performance)*\n\n"
            "Wings always uses HTTPS internally; No TLS Verify is required so Cloudflare "
            "can forward to the local HTTPS listener."
        ),
        inline=False,
    )
    e.add_field(
        name="4️⃣  SSL / TLS Mode",
        value="Set to **Full** in the Cloudflare dashboard. Do **not** use Flexible.",
        inline=False,
    )
    e.add_field(
        name="5️⃣  Panel Node Configuration",
        value=(
            ...
            "• **Scheme**: `https`\n"
            "• **Port**: `443`\n\n"
            "Paste the updated config to `/etc/pterodactyl/config.yml`, then:\n"
            "```bash\nsystemctl restart wings\n```"
        ),
        inline=False,
    )
    e.add_field(
        name="6️⃣  DNS (alternative — Free plan)",
        value=(
            "A record: `wings.yourdomain.com → <VPS IP>` → **grey cloud** (DNS-only)\n"
            "Then open port **8080** on your VPS firewall."
        ),
        inline=False,
    )
    e.add_field(
        name="✅  Result",
        value="Wings accessible at: **`https://wings.yourdomain.com`** (or `:8080` DNS-only).",
        inline=False,
    )
    _set_footer(e, logo)
    return e
