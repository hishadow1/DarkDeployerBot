...
async def create_docker_container(container_name, ram_mb, cpu_count, ssh_port, password, disk_gb=30, hostname=None, progress_msg=None, node_id=None):
    ...
    if not node_id or node_id == node_system.LOCAL_NODE_ID:
        ...
        # Remote node: check whether the image already exists there; build it
        # automatically if it doesn't (or if the Dockerfile has changed).
        img_out, _, _ = await _run_host_cmd(node_id, f"docker images -q {DOCKER_IMAGE}", timeout=30)
        if not img_out.strip():
            # Image is missing on the remote node — ship the Dockerfile and build.
            logger.info(
                f"[create_vps] {DOCKER_IMAGE} not found on remote node {node_id} — "
            ...
            # Encode the Dockerfile so it can be passed safely as a shell string.
            ...
            _build_cmd   = (
                ...
                f"echo '{_df_b64}' | base64 -d > {_remote_df} && "
            ...
            # remote_execute already raises RuntimeError if the command exits
            # non-zero (the agent sends success=False and we check it).  The
            # && before echo 'BUILD_OK' guarantees the echo only runs when
            # docker build exits 0, so a clean return here IS confirmation of
            # success — no need to grep the (potentially truncated) output.
