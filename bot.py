...
async def ensure_vps_image(force_rebuild: bool = False) -> tuple:
    ...
    async with _image_build_lock:
        ...
        # DOCKER_BUILDKIT=1: use the modern builder (built into Docker 23+).
        # This enables parallel layer execution, smarter caching, and cleaner
        # build output.  The legacy builder (=0) is no longer needed on any
...
_AI_ERROR_PATTERNS = [
    ...
    (["pterodactyl", "wings", "panel", "p: wings", "daemon"],
     ...
     "fix_wings",
     # 1. Rewrite wings.service with all hardened settings (ExecStartPre network
     #    cleanup, LimitNOFILE=65535, PartOf removed, StartLimitBurst=5).
     #    tee with heredoc so the whole unit lands atomically.
     "[ -f /usr/local/bin/wings ] || { echo 'Wings not installed — run /template to install it'; exit 0; }; "
     "cat > /etc/systemd/system/wings.service << 'WINGS_SVC_EOF'\n"
     "[Unit]\n"
     ...
     "LimitNOFILE=65535\n"
     r"ExecStartPre=/bin/bash -c 'docker network rm pterodactyl_nw 2>/dev/null || true; "
     ...
     # Wings creates the pterodactyl0 bridge interface itself.  A leftover
     # interface from a crashed run holds the 172.21.x.x subnet and causes
     # Docker to refuse a new network even after network objects are removed.
     ...
     "WINGS_SVC_EOF\n"
     # 2. Reload systemd so the new unit takes effect
     "systemctl daemon-reload; "
     # 3. Force-remove Docker networks overlapping 172.21.0.0/16, then delete
     #    any leftover pterodactyl0 bridge before restarting Wings.
     ...
     r"ip link set pterodactyl0 down 2>/dev/null || true; "
     r"ip link delete pterodactyl0 2>/dev/null || true; "
     # 4. Restart Wings with the repaired service file
     "systemctl restart wings 2>/dev/null && echo WINGS_RESTARTED || echo WINGS_FAILED"),
            ...
            if cfg.get("service_crash", True):
                try:
                    fail_out, _, _ = await docker_exec(
                        cname,
                        "systemctl list-units --state=failed --no-legend --no-pager 2>/dev/null | grep -oP '\\S+\\.service' | head -3",
                        timeout=10
                    )
                    failed_svcs = [s.strip() for s in fail_out.splitlines() if s.strip()]
                    if failed_svcs:
