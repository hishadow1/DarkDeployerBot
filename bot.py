...
# Gzipped tarball served at GET /image/darknodes-vps via the Cloudflare Tunnel.
# Written after every successful local build; remote nodes curl+load it instead
# of running a full docker build (~30 s–2 min download vs 15–20 min build).
_IMAGE_EXPORT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), ".darknodes_image_export.tar.gz")
...
# ─── Registry config (optional) ──────────────────────────────────────────────
# Set these three env vars to enable registry-based image distribution.
# After every local build the bot pushes the image; remote nodes pull it
# instead of running a full docker build (~1-2 min pull vs 15-20 min build).
#
#   REGISTRY_IMAGE    full image reference, e.g. ghcr.io/myuser/darknodes-vps
#                     or myuser/darknodes-vps (Docker Hub)
#   REGISTRY_USERNAME your registry username / GitHub username
#   REGISTRY_PASSWORD your registry password / personal access token
REGISTRY_IMAGE    = os.environ.get("REGISTRY_IMAGE", "").strip()
REGISTRY_USERNAME = os.environ.get("REGISTRY_USERNAME", "").strip()
REGISTRY_PASSWORD = os.environ.get("REGISTRY_PASSWORD", "").strip()
...
def _registry_configured() -> bool:
    """Return True if all three registry env vars are set."""
    return bool(REGISTRY_IMAGE and REGISTRY_USERNAME and REGISTRY_PASSWORD)


def _registry_host() -> str:
    """Return the registry hostname, or '' for Docker Hub."""
    first = REGISTRY_IMAGE.split("/")[0]
    return first if ("." in first or ":" in first) else ""


async def _registry_login_local() -> tuple[bool, str]:
    """Login to the registry on the local Docker daemon."""
    host = _registry_host()
    cmd = (
        f"echo {shlex.quote(REGISTRY_PASSWORD)} | "
        f"docker login -u {shlex.quote(REGISTRY_USERNAME)} --password-stdin {host}"
    ).rstrip()
...
async def _push_image_to_registry() -> None:
    """Tag + push the local VPS image to the registry (fire-and-forget task)."""
    if not _registry_configured():
        return
    try:
        ok, detail = await _registry_login_local()
        if not ok:
            logger.warning(f"[registry] Login failed: {detail}")
            return
        for cmd, label in [
            (f"docker tag {DOCKER_IMAGE} {REGISTRY_IMAGE}", "tag"),
            (f"docker push {REGISTRY_IMAGE}", "push"),
        ]:
            proc = await asyncio.create_subprocess_shell(
                cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.STDOUT,
            )
            ...
            if proc.returncode != 0:
                logger.warning(
                    f"[registry] {label} failed: {out.decode(errors='replace')[-400:]}"
                )
                return
        logger.info(f"[registry] Pushed {DOCKER_IMAGE} → {REGISTRY_IMAGE}")
    except Exception as exc:
        logger.warning(f"[registry] Push error: {exc}")


async def _pull_image_from_registry_remote(node_id: str) -> tuple[bool, str]:
    """Pull the VPS image on a remote node from the registry.

    Returns (True, detail) on success, (False, error) otherwise.
    Remote nodes that successfully pull the image skip the slow docker build.
    """
    if not _registry_configured():
    ...
    pull_cmd = (
        f"{login_part}"
        f"docker pull {REGISTRY_IMAGE} && "
        f"docker tag {REGISTRY_IMAGE} {DOCKER_IMAGE} && "
        f"echo 'PULL_OK'"
    )
    try:
        out = await node_system.remote_execute(node_id, pull_cmd, timeout=300)
        if "PULL_OK" in (out or ""):
            return True, "Pulled from registry"
        return False, (out or "no output")[-500:]
    except Exception as exc:
        return False, str(exc)[-300:]


async def _export_vps_image() -> None:
    """Export the local VPS image as a gzipped tarball for tunnel-based distribution.

    Called as a fire-and-forget task after every successful local build.
    Remote nodes fetch GET /image/darknodes-vps via the Cloudflare Tunnel and
    pipe the response into `docker load` — no registry credentials required.
    """
    try:
        logger.info(f"[image-export] Exporting {DOCKER_IMAGE} → {_IMAGE_EXPORT_PATH} …")
        tmp_path = _IMAGE_EXPORT_PATH + ".tmp"
        proc = await asyncio.create_subprocess_shell(
            f"docker save {DOCKER_IMAGE} | gzip > {shlex.quote(tmp_path)}",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.STDOUT,
        )
        _, _ = await asyncio.wait_for(proc.communicate(), timeout=900)
        if proc.returncode != 0:
            logger.warning(f"[image-export] Export failed (rc={proc.returncode})")
            try:
                os.remove(tmp_path)
            except Exception:
                pass
            return
        os.replace(tmp_path, _IMAGE_EXPORT_PATH)
        # Write the Dockerfile hash alongside the tarball so remote nodes can
        # check in ~1 KB whether their cached image is already up to date.
        with open(_IMAGE_EXPORT_PATH + ".hash", "w") as _efh:
            _efh.write(_dockerfile_hash())
        # Register path with Node Manager HTTP server so /image/darknodes-vps works.
        node_system.set_image_export_path(_IMAGE_EXPORT_PATH)
        _sz_mb = os.path.getsize(_IMAGE_EXPORT_PATH) / 1024 / 1024
        logger.info(
            f"[image-export] Export ready ({_sz_mb:.0f} MB) — "
            f"remote nodes can now load via tunnel at /image/darknodes-vps"
        )
    except asyncio.TimeoutError:
        logger.warning("[image-export] Export timed out (15 min)")
    except Exception as _exc:
        logger.warning(f"[image-export] Export error: {_exc}")
...
async def _run_container_cmd(node_id: str, container: str, command: str, timeout: int = 60):
    ...
    try:
        shell_cmd = f"docker exec {container} bash -c {shlex.quote(command)}"
        out = await node_system.remote_execute(node_id, shell_cmd, timeout=timeout)
        return out, "", 0
    except Exception as exc:
        return "", str(exc), 1
...
async def _unique_container_name(user_id: str, start_count: int, node_id: str = None) -> tuple:
    ...
    Docker container on the target node. Starts at `start_count` and increments
    until a free slot is found, so a stale/conflicting container never blocks a
    new deployment.
    """
    count = start_count
    while count < start_count + 50:  # safety cap — avoid infinite loops
        ...
        if node_id and node_id != node_system.LOCAL_NODE_ID:
            try:
                # The name must be checked on the node that will actually run
                # the container. Checking only the bot host lets remote Docker
                # report a name conflict during deployment.
                ...
                if "no such object" in inspect_text or "no such container" in inspect_text:
                    ...
                    # An offline/unreachable node is not the same as a free
                    # name; fail explicitly instead of masking the real issue.
                    raise
        else:
...
async def ensure_vps_image(force_rebuild: bool = False) -> tuple:
    ...
    async with _image_build_lock:
        ...
        build_cmd = (
            f"docker build "
            f"--network=host "
            f"--label 'darknodes.df-hash={current_hash}' "
            f"-t {DOCKER_IMAGE} "
            f"-f {DOCKERFILE_PATH} "
            f"{build_dir}"
        )
        # DOCKER_BUILDKIT=1 enables parallel layer execution, better caching,
        # and is required for --mount=type=cache directives in the Dockerfile.
        # Docker 20.10+ ships BuildKit built-in; no separate buildx plugin needed.
        ...
        try:
            ...
            # Export image for tunnel-based distribution (zero-config) and
            # optionally push to registry — both are fire-and-forget.
            asyncio.create_task(_export_vps_image())
            if _registry_configured():
                asyncio.create_task(_push_image_to_registry())
...
async def _ensure_remote_vps_image(node_id: str) -> tuple[bool, str]:
    ...
    async with _image_build_lock:
        ...
        # Primary check: local hash file matches (fast path after bot restart).
        if img_out.strip() and _current_df_hash == _remote_stored_hash:
            logger.info(
                f"[remote-image] {DOCKER_IMAGE} is ready on node {node_id} "
                f"(hash {_current_df_hash[:8]} — local file)"
            )
            return True, "Image is already up to date."

        # Secondary check: image exists but local hash file is missing/stale
        # (happens after bot restarts).  Read the hash label baked into the
        # image during the last build to avoid an unnecessary full rebuild.
        if img_out.strip() and _current_df_hash != _remote_stored_hash:
            _label_out, _, _label_rc = await _run_host_cmd(
                node_id,
                f"docker inspect --format '{{{{index .Config.Labels \"darknodes.df-hash\"}}}}' {DOCKER_IMAGE}",
                timeout=10,
            )
            _remote_label_hash = _label_out.strip()
            if _remote_label_hash == _current_df_hash:
                logger.info(
                    f"[remote-image] {DOCKER_IMAGE} is ready on node {node_id} "
                    f"(hash {_current_df_hash[:8]} — image label)"
                )
                # Refresh local hash file so next check is instant.
                try:
                    with open(_remote_hash_file, "w") as _rhf:
                        _rhf.write(_current_df_hash)
                except Exception:
                    pass
                return True, "Image is already up to date (verified via label)."

        # ── Try tunnel-based load (zero-config — uses existing Cloudflare Tunnel) ─
        # The bot exports the image to a tarball after every local build and
        # serves it at /image/darknodes-vps on the Node Manager HTTP server.
        # Remote nodes download and docker-load it: typically 30 s–2 min vs
        # 15–20 min for a full build.  Falls back silently if unavailable.
        _tunnel_base = node_system._tunnel_url() if node_system._tunnel_configured() else ""
        if _tunnel_base and os.path.exists(_IMAGE_EXPORT_PATH):
            _img_url  = f"{_tunnel_base}/image/darknodes-vps"
            _hash_url = f"{_tunnel_base}/image/darknodes-vps.hash"
            logger.info(
                f"[remote-image] Trying tunnel-based load for node {node_id} "
                f"({_img_url}) …"
            )
            # One shell script: fast hash check → skip or download+load
            _load_cmd = (
                f"_srv=$(curl -fsSL --connect-timeout 10 --max-time 10 "
                f"  {shlex.quote(_hash_url)} 2>/dev/null || true); "
                f"_local=$(docker inspect --format "
                f"  '{{{{index .Config.Labels \"darknodes.df-hash\"}}}}' "
                f"  {DOCKER_IMAGE} 2>/dev/null || true); "
                f"if [ -n \"$_srv\" ] && [ \"$_local\" = \"$_srv\" ]; then "
                f"  echo 'LOAD_SKIP'; "
                f"else "
                f"  curl -fsSL --connect-timeout 30 --max-time 600 "
                f"    {shlex.quote(_img_url)} | docker load && echo 'LOAD_OK'; "
                f"fi"
            )
            try:
                _load_out, _load_err, _load_rc = await _run_host_cmd(
                    node_id, _load_cmd, timeout=650
                )
                _load_combined = (_load_out or "") + (_load_err or "")
                if _load_rc == 0 and (
                    "LOAD_OK" in _load_combined or "LOAD_SKIP" in _load_combined
                ):
                    try:
                        with open(_remote_hash_file, "w") as _rhf:
                            _rhf.write(_current_df_hash)
                        ...
                        pass
                    logger.info(
                        f"[remote-image] Tunnel load OK for node {node_id} "
                        f"({'skip — already current' if 'LOAD_SKIP' in _load_combined else 'downloaded+loaded'})."
                    )
                    return True, "Image loaded via tunnel"
                logger.warning(
                    f"[remote-image] Tunnel load failed for node {node_id} "
                    f"(rc={_load_rc}, {_load_combined[-200:]}) — trying next fallback"
                )
            except Exception as _te:
                logger.warning(
                    f"[remote-image] Tunnel load error for node {node_id}: {_te} "
                    "— trying next fallback"
                )

        # ── Try registry pull before building (much faster than a full build) ───
        if _registry_configured():
            logger.info(
                f"[remote-image] Trying registry pull for {node_id} "
                f"({REGISTRY_IMAGE}) …"
            )
            _pull_ok, _pull_detail = await _pull_image_from_registry_remote(node_id)
            if _pull_ok:
                # Tag with Dockerfile hash so subsequent checks skip the pull
                _tag_cmd = (
                    f"docker inspect --format '{{{{index .Config.Labels \"darknodes.df-hash\"}}}}' "
                    f"{DOCKER_IMAGE} 2>/dev/null || true"
                )
                try:
                    with open(_remote_hash_file, "w") as _rhf:
                    ...
                    pass
                logger.info(
                    f"[remote-image] Registry pull succeeded on node {node_id}."
                )
                return True, "Pulled from registry"
            logger.warning(
                f"[remote-image] Registry pull failed on node {node_id} "
                f"({_pull_detail[:200]}) — falling back to docker build"
            )
        ...
        _build_cmd = (
            ...
            f"DOCKER_BUILDKIT=1 docker build --network=host "
            f"--label 'darknodes.df-hash={_current_df_hash}' "
        ...
        try:
            ...
            if remote_nodes:
                for node_id, _node in remote_nodes:
                    ...
                    ok, detail = await _ensure_remote_vps_image(node_id)
                    if ok:
                        logger.info(f"[remote-image] Warm-up ready for node {node_id}.")
                    else:
                        logger.warning(
                            f"[remote-image] Warm-up failed for node {node_id}: {detail[:500]}"
                        )
...
async def create_docker_container(container_name, ram_mb, cpu_count, ssh_port, password, disk_gb=30, hostname=None, progress_msg=None, node_id=None):
    """
...
def _is_container_unavailable(err: str) -> bool:
    """Return True if the error string indicates the container doesn't exist or isn't running."""
    _e = err.lower()
    return any(k in _e for k in (
        "no such container",
        "is not running",
...
async def _ensure_container_running(node_id: str, container_name: str) -> None:
    """
    Make sure the container is running on the given node.

    If the container is stopped, attempts `docker start` and waits up to 20 s.
    Raises RuntimeError if the container cannot be started or doesn't exist.
    """
    # Check current state via docker inspect
    state_out, state_err, state_rc = await _run_host_cmd(
        node_id,
    ...
    if state_rc != 0 or state in ("", "missing"):
        # Container doesn't exist on this node
        raise RuntimeError(
            f"Container `{container_name}` was not found on this node. "
            "The VPS may have been removed or migrated."
        )

    if state in ("exited", "stopped", "created", "dead"):
        logger.info(f"[session] Container {container_name} is {state} — starting it…")
        start_out, start_err, start_rc = await _run_host_cmd(
            node_id,
        ...
        if start_rc != 0:
            raise RuntimeError(
                f"Failed to start container `{container_name}`: {(start_out + start_err)[:200]}"
            )
        # Wait up to 20 s for the container to reach running state
        for _ in range(20):
            await asyncio.sleep(1)
            chk_out, _, chk_rc = await _run_host_cmd(
                node_id,
                f"docker inspect --format '{{{{.State.Status}}}}' {shlex.quote(container_name)}",
                timeout=5,
            )
            if chk_out.strip().lower() == "running":
                logger.info(f"[session] Container {container_name} is now running.")
                return
        raise RuntimeError(
            f"Container `{container_name}` did not reach running state within 20s after start."
        )

    # Some other state (pausing, restarting, etc.) — best-effort wait
    for _ in range(10):
        await asyncio.sleep(1)
        chk_out, _, chk_rc = await _run_host_cmd(
            node_id,
        ...
        if chk_out.strip().lower() == "running":
            return
    raise RuntimeError(
        f"Container `{container_name}` is in state `{state}` and did not become running."
    )
...
async def get_tmate_session(container_name: str, node_id: str = None) -> dict:
    ...
    # Ensure the container is running before attempting docker exec.
    # This catches "stopped after systemd crash + restart" races that show up
    # as "No such container" or "container not running" from docker exec.
    await _ensure_container_running(_eff_node_id, container_name)

    chk_out, chk_err, chk_rc = await _run_container_cmd(
        _eff_node_id, container_name, "command -v tmate", timeout=10
    )
    if chk_rc != 0:
        # Distinguish "container unavailable" from "tmate not installed"
        _chk_combined = chk_out + chk_err
        if _is_container_unavailable(_chk_combined):
            raise Exception(
                f"Container `{container_name}` is not accessible (docker exec failed): "
                f"{_chk_combined[:200]}"
            )
...
"""
        ...
        if inst_rc != 0 and _is_container_unavailable(inst_out + inst_err):
            raise Exception(
                f"Container `{container_name}` became unavailable during tmate install. "
                f"Detail: {(inst_out + inst_err)[:200]}"
            )
...
async def get_sshx_session(container_name: str, node_id: str = None) -> str:
    ...
    if chk_rc != 0:
        ...
        if install_rc != 0 and _is_container_unavailable(install_out + install_err):
            raise Exception(
                f"Container `{container_name}` became unavailable during sshx install. "
                f"Detail: {(install_out + install_err)[:200]}"
            )
...
async def on_ready():
    ...
    # Re-register the image export path immediately on startup so the HTTP
    # endpoint works right away if the tarball survived the restart.
    if os.path.exists(_IMAGE_EXPORT_PATH):
        node_system.set_image_export_path(_IMAGE_EXPORT_PATH)
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if action == 'reinstall':
            ...
            try:
                ...
                tmate_err = ""
                try:
                    tmate_info = await get_tmate_session(container_name, node_id=_node_id)
                ...
                sshx_err = ""
                try:
                    sshx_url = await get_sshx_session(container_name, node_id=_node_id)
...
async def create_vps(ctx, user: discord.Member, ram: int, cpu: int, disk: int = 30):
    ...
    try:
        container_name = await create_docker_container(container_name, ram_mb, cpu, ssh_port, password, disk_gb=disk, hostname=vps_hostname, progress_msg=progress_msg, node_id=deploy_node_id)
        ...
        tmate_info = {}
        try:
            tmate_info = await get_tmate_session(
                container_name, node_id=deploy_node_id
            )
        ...
        sshx_url = ""
        try:
            sshx_url = await get_sshx_session(
                container_name, node_id=deploy_node_id
            )
...
async def buy_with_credits(ctx, plan: str, processor: str = "Intel"):
    ...
    ssh_port = get_next_ssh_port()
    try:
        container_name = await create_docker_container(container_name, ram_mb, cpu_str, ssh_port, password, hostname=vps_hostname, progress_msg=progress_msg, node_id=deploy_node_id)
        ...
        for _vps in _vpss:
            if _vps.get("status") == "running":
                ...
                if _cn:
                    ...
                    # Update the stored hostname (e.g. "OldBrand-2" → "NewBrand-2")
                    if _old_hostname:
