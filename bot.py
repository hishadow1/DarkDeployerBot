...
async def create_docker_container(container_name, ram_mb, cpu_count, ssh_port, password, disk_gb=30, hostname=None, progress_msg=None, node_id=None):
    ...
    try:
        if not node_id or node_id == node_system.LOCAL_NODE_ID:
            # Use compat wrapper: auto-retries on rlimit and OCI/runc errors
            await _docker_run_compat(run_cmd, timeout=60)
        else:
            # Remote node: attempt once; on OCI/runc failure, retry with cgroupns=private
            try:
                await node_system.remote_execute(node_id, run_cmd, timeout=60)
            except Exception as _remote_err:
                _re_str = str(_remote_err).lower()
                if any(k in _re_str for k in ("runc", "oci runtime", "shim", "container init")):
                    _fallback_run = run_cmd.replace("--cgroupns=host", "--cgroupns=private")
                    if _fallback_run != run_cmd:
                        logger.warning(
                            f"[docker run] Remote OCI/runc error — retrying with --cgroupns=private: "
                            f"{str(_remote_err)[:200]}"
                        )
                        await node_system.remote_execute(node_id, _fallback_run, timeout=60)
                    else:
                        raise
                else:
                    raise
...
def _register_missing_slash_commands() -> None:
    ...
    for prefix_command in sorted(bot.commands, key=lambda command: command.name):
        ...
        source = (
            ...
            + "):\n"
            "    return await __handler(interaction, **locals())\n"
        )
