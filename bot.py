...
_AI_ERROR_PATTERNS = [
    ...
    (["pterodactyl", "wings", "panel", "p: wings", "daemon"],
     ...
     # 1a. Write the pre-start cleanup script as a real file.
     #     IMPORTANT: do NOT inline this in ExecStartPre=/bin/bash -c '...'.
     #     systemd expands $VAR even inside single-quoted Exec* values, turning
     #     all bash variables ($net, $subnet, $cid …) into empty strings and
     #     making the cleanup loop completely inert.
     "cat > /usr/local/bin/wings-pre-start.sh << 'WPSS_EOF'\n"
     "#!/bin/bash\n"
     ...
     '    echo "$subnet" | grep -qE "172\\.21\\." || continue\n'
     '    for cid in $(docker network inspect "$net" --format "{{range $k,$v := .Containers}}{{$v.Name}} {{end}}" 2>/dev/null); do\n'
     ...
     # 1b. Write the wings.service using the script file for ExecStartPre.
     ...
     # 3. Run the cleanup script once right now (so we don't have to wait for
     #    the next Wings start to clear the leftover bridge/networks).
     "/usr/local/bin/wings-pre-start.sh 2>/dev/null || true; "
