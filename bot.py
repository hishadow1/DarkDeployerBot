...
async def _update_container_brand(container_name: str, brand: str, old_hostname: str = None) -> None:
    """
    ...
    try:
        ...
        if old_hostname:
            ...
            if m:
                new_hostname = f"{brand}-{m.group(1)}"

        if new_hostname:
            old_esc = re.escape(old_hostname)
            script = (
                f"echo {shlex.quote(brand)} > /etc/darknodes-brand; "
                f"echo {shlex.quote(new_hostname)} > /etc/hostname; "
                f"sed -i 's/{old_esc}/{new_hostname}/g' /etc/hosts 2>/dev/null || true; "
                f"grep -qF {shlex.quote(new_hostname)} /etc/hosts || "
                f"  echo '127.0.1.1 {new_hostname}' >> /etc/hosts; "
                f"hostname {shlex.quote(new_hostname)}; "
                f"sed -i 's/{old_esc}/{new_hostname}/g' /etc/motd 2>/dev/null || true"
            )
        else:
            script = f"echo {shlex.quote(brand)} > /etc/darknodes-brand"
        ...
        pass   # silent — offline containers update on next deploy
...
async def _docker_run_compat(run_cmd: str, timeout: int = 60) -> None:
    """
    Execute a 'docker run' command.  If the host kernel rejects --ulimit nproc
    (OCI runtime 'error setting rlimit'), automatically retry without that flag
    so containers still start on restricted hosts while keeping the higher limit
    on hosts that support it.
    """
    try:
        await execute_docker(run_cmd, timeout=timeout)
    except Exception as _run_err:
        if "rlimit" in str(_run_err).lower():
            logger.warning(
...
async def create_docker_container(container_name, ram_mb, cpu_count, ssh_port, password, disk_gb=30, hostname=None, progress_msg=None, node_id=None):
    ...
    if not node_id or node_id == node_system.LOCAL_NODE_ID:
        ...
        # Remote node: check whether the image already exists there AND whether
        # the Dockerfile has changed since the last build.  Rebuild if either
        # condition is true (mirrors the local ensure_vps_image logic).
        ...
        # Load stored hash for this remote node
        _remote_stored_hash = ""
        try:
        ...
        # 1a. Verify Docker daemon is reachable on the remote before anything else
        ...
        if _docker_ping_rc != 0:
            ...
            raise RuntimeError(
                ...
                "Ensure Docker is installed and the daemon is running on the remote machine."
            )

        img_out, _, _ = await _run_host_cmd(node_id, f"docker images -q {DOCKER_IMAGE}", timeout=30)
        _remote_image_exists = bool(img_out.strip())
        _df_changed = _current_df_hash != _remote_stored_hash

        if not _remote_image_exists or _df_changed:
            _rebuild_reason = (
                "image not found on remote node"
                if not _remote_image_exists
                else "Dockerfile changed — rebuilding on remote"
            )
            logger.info(
                f"[create_vps] {DOCKER_IMAGE} on remote node {node_id}: {_rebuild_reason} "
                "(uploading Dockerfile and triggering remote build — this may take ~10 min) …"
            )
            if not os.path.exists(DOCKERFILE_PATH):
                await _progress(0, failed=True)
                raise RuntimeError(
                    f"Cannot build image on remote node — "
                    f"Dockerfile not found locally at `{DOCKERFILE_PATH}`."
                )
            # Encode the Dockerfile so it can be passed safely as a shell argument.
            # base64 output uses only A-Za-z0-9+/= — no quoting issues.
            ...
            # Use printf instead of echo to avoid ARG_MAX issues on very long strings.
            # Cleanup always runs (success or failure) via the trap.
            _build_cmd = (
                f"trap 'rm -rf {_remote_ctx} {_remote_df}' EXIT; "
                f"mkdir -p {_remote_ctx} && "
                f"printf '%s' '{_df_b64}' | base64 -d > {_remote_df} && "
                f"DOCKER_BUILDKIT=0 docker build "
                f"--network=host "
                f"-t {DOCKER_IMAGE} "
                f"-f {_remote_df} "
                f"{_remote_ctx} "
            ...
            try:
                ...
                await _progress(0, failed=True)
                _be_str = str(_be)
                # Show a generous tail — docker build output is long and the
                # relevant failure is always near the end.
                _err_tail = _be_str[-4000:] if len(_be_str) > 4000 else _be_str
                raise RuntimeError(
                    f"Failed to build `{DOCKER_IMAGE}` on remote node `{node_id}`.\n\n"
                    f"{_err_tail}"
                )
            # remote_execute raises RuntimeError on non-zero exit, so reaching
            # here means docker build exited 0.  Persist the hash so future
            # deploys skip the rebuild when Dockerfile hasn't changed.
            try:
                with open(_remote_hash_file, "w") as _rhf:
                    _rhf.write(_current_df_hash)
            except Exception as _he:
                logger.warning(f"[create_vps] Could not save remote image hash: {_he}")
            logger.info(f"[create_vps] Remote image build complete on node {node_id}.")
        else:
            logger.info(
                f"[create_vps] {DOCKER_IMAGE} image is current on remote node {node_id} "
                f"(hash {_current_df_hash[:8]}) — skipping rebuild."
            )
    ...
    async def _collect_diagnostics(container: str) -> str:
        logs_out,    _, _ = await _run_host_cmd(_eff_node_id, f"docker logs --tail=80 {container}",   timeout=20)
        inspect_out, _, _ = await _run_host_cmd(_eff_node_id, f"docker inspect {container}",           timeout=15)
        # Pull docker.service journal from INSIDE the container
    ...
    try:
        if not node_id or node_id == node_system.LOCAL_NODE_ID:
            ...
            await node_system.remote_execute(node_id, run_cmd, timeout=60)
...
# If it does not respond within 20 s we assume the host kernel doesn't support
# overlay inside privileged containers and fall back to fuse-overlayfs.
_docker_waited=0
while [ $_docker_waited -lt 20 ]; do
...
if ! docker info >/dev/null 2>&1; then
    echo "overlay2 not ready after 20 s — switching to fuse-overlayfs …"
...
DEOF
    ...
    _fuse_waited=0
    while [ $_fuse_waited -lt 90 ]; do
        if docker info >/dev/null 2>&1; then
            echo "Docker ready (fuse-overlayfs) after $_fuse_waited s."
            break
        fi
        if [ "$_fuse_waited" -eq 30 ]; then
            systemctl restart docker 2>/dev/null || true
        fi
        sleep 1
...
async def get_tmate_session(container_name: str) -> dict:
    ...
    """
    _, _, chk_rc = await docker_exec(
        container_name, "command -v tmate", timeout=10
    )
    if chk_rc != 0:
...
tmate -V
"""
        inst_out, inst_err, inst_rc = await docker_exec(
            container_name, install_script, timeout=120
        )
        # Re-check after install attempt
        _, _, chk_rc2 = await docker_exec(container_name, "command -v tmate", timeout=10)
        if chk_rc2 != 0:
            raise Exception(
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        ...
        if action == 'reinstall':
            ...
            try:
                ...
                try:
                    _chk_proc = await asyncio.create_subprocess_exec(
                        "docker", "inspect", "--format", "{{.State.Paused}}", container_name,
                        stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
                    _chk_out, _ = await asyncio.wait_for(_chk_proc.communicate(), timeout=10)
                    if _chk_out.decode().strip() == "true":
                        logger.info(f"[ssh] Container {container_name} is paused — unpausing before session start")
                        await execute_docker(f"docker unpause {container_name}", timeout=15)
                        await asyncio.sleep(2)
            ...
            try:
                ...
                class FileManagerView(discord.ui.View):
                    ...
                    async def _browse(self_fmv, inter2: discord.Interaction, path: str):
                        ...
                        ls2, _, _ = await docker_exec(container_name, f"ls -lhA {path} 2>/dev/null | head -30", timeout=15)
                            ...
                            async def on_submit(self_dm, inter3: discord.Interaction):
                                ...
                                if target in ("/", "/etc", "/usr", "/bin", "/sbin", "/lib", "/var", "/proc", "/sys"):
                                    await inter3.followup.send(
                                        ...
                                        ephemeral=True)
                                    return
                                check_out, _, check_rc = await docker_exec(
                                ...
                                if "NOTFOUND" in check_out:
                                    await inter3.followup.send(
                                        ...
                                        ephemeral=True)
                                    return
                                del_out, del_err, del_rc = await docker_exec(
                    ...
                    async def new_folder(self_fmv, inter2: discord.Interaction, btn: discord.ui.Button):
                        ...
                        class MkdirModal(discord.ui.Modal, title="📁 Create New Folder"):
                            ...
                            async def on_submit(self_mm, inter3: discord.Interaction):
                                ...
                                mk_out, mk_err, mk_rc = await docker_exec(
                    ...
                    async def upload_help(self_fmv, inter2: discord.Interaction, btn: discord.ui.Button):
                        ...
                        em2 = create_info_embed(
                            ...
                            "To upload a file directly into your VPS, use the `!upload` command:")
                        em2.add_field(
                            name="📋 Steps",
                            value=(
                                f"1. Type `!upload {_vps_idx} /root/` in this channel\n"
                                f"2. **Attach the file** to that same message\n"
                                f"3. The file will be placed at `/root/<filename>` on your VPS\n\n"
                                f"**Example:**\n`!upload {_vps_idx} /root/myconfig.yml` *(with file attached)*"
                            ),
                            inline=False
                        )
                        em2.add_field(
                            name="📥 Download a File",
                            value=f"`!download {_vps_idx} /path/to/file` — sends the file to Discord",
                            inline=False
                        )
                        em2.add_field(
                            name="✏️ View & Edit a File",
                            value=f"`!editfile {_vps_idx} /path/to/file` — view and edit inline",
                            inline=False
                        )
                        await inter2.followup.send(embed=em2, ephemeral=True)
            ...
            try:
                ...
                embed.set_footer(text="⬇ Download = save backup file  •  🔄 Restore = roll back VPS to this snapshot")
                ...
                class BackupListView(discord.ui.View):
                    def __init__(self_blv):
                        super().__init__(timeout=120)
                        # Show max 3 backups (2 buttons each = 6 total, fits in 2 rows)
                        for b_name, b_size, b_created in backups[:3]:
                            short = b_name.split('-backup-')[-1][:18] if '-backup-' in b_name else b_name[:18]
                            dl_btn = discord.ui.Button(label=f"⬇ {short}", style=discord.ButtonStyle.primary)
                            dl_btn.callback = self_blv._make_dl_cb(b_name)
                            restore_btn = discord.ui.Button(label=f"🔄 Restore", style=discord.ButtonStyle.danger)
                            restore_btn.callback = self_blv._make_restore_cb(b_name)
                            self_blv.add_item(dl_btn)
                            self_blv.add_item(restore_btn)

                    def _make_dl_cb(self_blv, image_name):
                        async def callback(inter: discord.Interaction):
                            ...
                            tmp_path = f"/tmp/{safe_name}.tar.gz"
                            try:
                                ...
                                if file_size_mb <= 25:
                                    # Small enough — send directly via Discord
                                    await inter.followup.send(
                                        file=discord.File(tmp_path, filename=f"{safe_name}.tar.gz"),
                                        ephemeral=True)
                                    try:
                                        os.remove(tmp_path)
                                    except Exception:
                                    ...
                                    # Too large for Discord — upload to transfer.sh and provide a link
                                    await inter.followup.send(
                                        embed=create_info_embed("☁️ Uploading to Cloud…",
                                            f"Backup is **{file_size_mb:.0f} MB** (Discord limit: 25 MB).\n"
                                            f"Uploading to **transfer.sh** — you'll get a download link shortly."),
                                        ephemeral=True)
                                    upload_proc = await asyncio.create_subprocess_shell(
                                        f"curl -s --upload-file {tmp_path} "
                                        f"https://transfer.sh/{safe_name}.tar.gz",
                                        stdout=asyncio.subprocess.PIPE,
                                        stderr=asyncio.subprocess.PIPE)
                                    upload_stdout, upload_stderr = await asyncio.wait_for(
                                        upload_proc.communicate(), timeout=300)
                                    download_url = upload_stdout.decode().strip()
                                    if upload_proc.returncode == 0 and download_url.startswith("http"):
                                        dl_embed = create_success_embed(
                                            "☁️ Backup Ready to Download",
                                            f"Your backup has been uploaded securely.\n"
                                            f"Click the link below to download it:")
                                        dl_embed.add_field(name="📦 Backup", value=f"`{image_name}`", inline=True)
                                        dl_embed.add_field(name="💾 Size", value=f"`{file_size_mb:.0f} MB`", inline=True)
                                        dl_embed.add_field(name="⬇ Download Link",
                                            value=f"[**Click here to download**]({download_url})\n`{download_url}`",
                                            inline=False)
                                        dl_embed.add_field(name="⏳ Expiry",
                                            value="Link expires in **14 days** — download it before then.",
                                            inline=False)
                                        dl_embed.set_footer(text="Hosted by transfer.sh — keep this link private")
                                        await inter.followup.send(embed=dl_embed, ephemeral=True)
                                    else:
                                        err_detail = upload_stderr.decode()[:200] or "transfer.sh returned empty response"
                                        await inter.followup.send(
                                            embed=create_error_embed("Upload Failed",
                                                f"Could not upload backup to transfer.sh.\n`{err_detail}`\n\n"
                                                f"The backup file is at: `{tmp_path}` on the host server."),
                                            ephemeral=True)
                                    # Clean up temp file
                                    try:
                                        os.remove(tmp_path)
            ...
            class HelpModal(discord.ui.Modal, title="🆘 Describe Your Issue"):
                ...
                async def on_submit(self_m, inter: discord.Interaction):
                    ...
                    if best:
                        # KB matches found — build results embed + attach online references
                        result_embed = create_embed(
                            "🆘 Possible Solutions",
                            f"Found **{len(best)}** match(es) for your issue. "
                            f"Click a button to apply the fix to your VPS.",
                            0x5865F2,
                        )
                        ...
                        # Wait briefly for the online search (already running); add links if ready
                        try:
                            online = await asyncio.wait_for(
                                asyncio.shield(_search_task), timeout=5
                            )
                        except (asyncio.TimeoutError, Exception):
                            online = []
                        if online:
                            links = "\n".join(
                                f"• [{r['title'][:70]}]({r['url']})" for r in online[:3]
                            )
                            result_embed.add_field(
                                name="🌐 Online References",
                                value=links,
                                inline=False,
                            )

                        result_embed.set_footer(text="Fixes run inside your VPS — results shown after completion")
                    ...
                    if online:
                        fixable_online = [r for r in online if r.get("commands")]
                        has_fixes      = bool(fixable_online)

                        embed = create_embed(
                            "🌐 Online Solutions Found",
                            f"Found **{len(online)}** result(s) on StackExchange for:\n"
                            f"> _{self_m.issue.value[:100]}_\n\n"
                            + (
                                "Commands were extracted from the top answers. "
                                "Click **⚡ Apply Fix** to run them directly on your VPS, "
                                "or **🔗 View** to read the full solution first."
                                if has_fixes else
                                "Click **🔗 View** to read the full solution, "
                                "then use **🔑 SSH** to apply it manually."
                            ),
                            0x1ABC9C,
                        )

                        for idx, r in enumerate(online):
                            site_icon = "🔵" if r["site"] == "stackoverflow" else "🟠"
                            tags_str  = "  `" + "`  `".join(r["tags"][:3]) + "`" if r.get("tags") else ""
                            cmds      = r.get("commands", [])
                            fix_note  = f"\n✅ *{len(cmds)} command(s) extracted — Apply Fix available*" if cmds else ""
                            embed.add_field(
                                name=f"{site_icon} {idx+1}. {r['title'][:72]}",
                                value=f"[View Solution ↗]({r['url']}) • ⬆ {r['score']}{tags_str}{fix_note}",
                                inline=False,
                            )
                            # Preview the first extracted command in a code block
                            if cmds:
                                embed.add_field(
                                    name="📋 Extracted Command",
                                    value=f"```bash\n{cmds[0][:300]}\n```",
                                    inline=False,
                                )
                        ...
                        # Build view: for each result with commands add an Apply Fix button,
                        # plus a View link button. Discord allows max 25 components.
                        class OnlineSolveView(discord.ui.View):
                            def __init__(self_ov):
                                super().__init__(timeout=300)
                                for r in online[:5]:
                                    # Link button always present
                                    self_ov.add_item(discord.ui.Button(
                                        label=f"🔗 View: {r['title'][:45]}",
                                        url=r["url"],
                                        style=discord.ButtonStyle.link,
                                        row=online.index(r) % 5,
                                    ))
                                    # Apply Fix button only when commands were extracted
                                    if r.get("commands"):
                                        btn = discord.ui.Button(
                                            label=f"⚡ Apply Fix: {r['title'][:35]}",
                                            style=discord.ButtonStyle.danger,
                                            row=online.index(r) % 5,
                                        )
                                        btn.callback = self_ov._make_apply_cb(r)
...
async def create_vps(ctx, user: discord.Member, ram: int, cpu: int, disk: int = 30):
    ...
    try:
        await create_docker_container(container_name, ram_mb, cpu, ssh_port, password, disk_gb=disk, hostname=vps_hostname, progress_msg=progress_msg, node_id=deploy_node_id)
...
async def buy_with_credits(ctx, plan: str, processor: str = "Intel"):
    ...
    ssh_port = get_next_ssh_port()
    try:
        await create_docker_container(container_name, ram_mb, cpu_str, ssh_port, password, hostname=vps_hostname, progress_msg=progress_msg, node_id=deploy_node_id)
    ...
    for _uid, _vpss in vps_data.items():
        for _vps in _vpss:
            if _vps.get("status") == "running":
                ...
                if _cn:
                    asyncio.create_task(_update_container_brand(_cn, name, _old_hostname))
                    # Update the stored hostname (e.g. "OldBrand-2" → "NewBrand-2")
                    if _old_hostname:
                        _m = re.search(r'-(\d+)$', _old_hostname)
                        if _m:
                            _vps["hostname"] = f"{name}-{_m.group(1)}"
...
async def _do_vps_scan(container_name: str):
    ...
    async def _chk_disk():
        iss, fx = [], {}
        try:
            disk_out, _, _ = await docker_exec(
                container_name,
    ...
    async def _chk_dind():
        iss, fx = [], {}
        try:
            dind_out, _, dind_rc = await docker_exec(
                container_name,
