...
HEARTBEAT_SECS    = 15      # keep liveness independent of long-running Docker jobs
POLL_SECS         = 1       # seconds between job-poll cycles (keep low: reduces per-job overhead on remote nodes)
...
def _execute_job(command: str, timeout: int) -> tuple[bool, str]:
    try:
        ...
        # Keep the head (for context on failures) and the tail (for success
        # markers like BUILD_OK that appear at the very end of long outputs).
        if len(output) > 8000:
            output = output[:3000] + "\n…(output truncated)…\n" + output[-5000:]
...
def _heartbeat_loop(
    ...
    """Send heartbeats while the main loop is busy executing a remote job.

    Docker image builds and container creation can take several minutes.  The
    old single-threaded polling loop stopped sending heartbeats for the whole
    duration of those commands, so the manager correctly (but incorrectly for
    this case) marked a busy node offline after its liveness window expired.
    """
    while not stop_event.is_set():
        try:
            ...
            logger.warning(f"Heartbeat failed while agent was running: {exc}")
...
def _decode_rnode_token(rnode: str) -> dict | None:
    """Decode an RNODE_xxx reconnect token.  Returns {token_code, manager_url}."""
    try:
...
def reconnect_via_token(rnode_token: str, config_path: str) -> dict:
    """
    Reconnect to the Node Manager using a short-lived RNODE reconnect token.

    Restores the node's existing identity — no new node is created.
    All existing VPSes and settings on the bot side are preserved.

    Returns the config dict with node_id, node_api_key, manager_url.
    Saves config to disk so future restarts use the refreshed credentials.
    """
    decoded = _decode_rnode_token(rnode_token)
    if not decoded or not decoded.get("manager_url") or not decoded.get("token_code"):
        raise RuntimeError(
            "Invalid RNODE token — must start with RNODE_.\n"
            "Run /node reconnect in Discord to get a fresh token."
        )

    manager_url = decoded["manager_url"].rstrip("/")
    token_code  = decoded["token_code"]

    logger.info(f"Reconnecting to Node Manager at {manager_url} ...")

    resp = _http(
        "POST",
        f"{manager_url}/api/reconnect",
    ...
    if not node_id or not node_api_key:
        raise RuntimeError(f"Reconnect failed — unexpected server response: {resp}")

    # Merge with any existing local config to preserve hostname etc.
    existing = _load_config(config_path)
    cfg = {
        **existing,
        "node_id":        node_id,
        "node_api_key":   node_api_key,
        "manager_url":    resp.get("manager_url", manager_url),
        "hostname":       resp.get("hostname", existing.get("hostname", socket.gethostname())),
        "reconnected_at": datetime.utcnow().isoformat(),
    }
    _save_config(config_path, cfg)
    logger.info(f"Reconnect complete. Node ID: {node_id}")
    return cfg
...
def run(cfg: dict) -> None:
    """
    Main agent loop.
    • Heartbeat every HEARTBEAT_SECS in a separate worker, including during jobs.
    • Poll for jobs every POLL_SECS, execute them, post results.
    • VPS metadata sync every 5 minutes.
    """
    global _active_heartbeat_stop
    ...
    last_vsync    = 0.0
    consecutive   = 0
    # If the outer crash-recovery loop is starting a fresh run, stop the
    # previous worker first.  This prevents duplicate heartbeats after a
    # transient polling exception restarts the main loop.
    if _active_heartbeat_stop is not None:
        _active_heartbeat_stop.set()
    heartbeat_stop = threading.Event()
    _active_heartbeat_stop = heartbeat_stop
    heartbeat_thread = threading.Thread(
        target=_heartbeat_loop,
        args=(manager_url, auth, heartbeat_stop),
        name="darknodes-heartbeat",
        daemon=True,
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="",
        metavar="TOKEN",
        help="RNODE reconnect token from /node reconnect in Discord (manual recovery)",
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--register-only",
        action="store_true",
        help="Register with the manager, save credentials, then exit (used by install.sh)",
    )
    args = parser.parse_args()

    config_path = args.config

    # ── Reconnect mode (RNODE token from /node reconnect) ─────────────────────
    if args.reconnect_token.strip():
        try:
            cfg = reconnect_via_token(args.reconnect_token.strip(), config_path)
        except Exception as exc:
            print(f"ERROR: Reconnect failed: {exc}", file=sys.stderr)
            sys.exit(1)

        if args.register_only:
            logger.info("Reconnect complete — exiting (--register-only mode).")
            sys.exit(0)

    # ── Normal mode: load saved credentials or register with DNODE token ──────
    else:
        ...
        # ── Register-only mode: exit cleanly after saving credentials ─────────
        if args.register_only:
            logger.info("Registration complete — exiting (--register-only mode).")
            sys.exit(0)
