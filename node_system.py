...
_tokens:         Dict[str, Dict] = {}   # {code: {thread_id, channel_id, expires_at}}
_cmd_results:    Dict[str, Dict] = {}   # {cmd_id: {success, output}}
_thread_cursors: Dict[str, str]  = {}   # {thread_id: last_message_id}
_node_was_online: Dict[str, bool] = {}  # {node_id: bool} — tracks prev online state for change detection
...
async def _process_message(content: str) -> None:
    ...
    if content.startswith("REG:"):
        ...
        # Notify the channel where /node add was run
        ...
        if _bot and channel_id:
            try:
                ...
                logger.debug(f"[nodes] Could not send connect notification: {exc}")
...
async def _offline_monitor_loop() -> None:
    """
    Check every 30 s whether any remote node has changed online/offline state
    and post a notification to its relay thread when it does.
    """
    await asyncio.sleep(30)   # let nodes settle on startup
    while True:
        try:
                ...
                if currently_online:
                    ...
                    if thread_id:
                        try:
                            ...
                            await _post_embed_to_thread(thread_id, e)
                        except Exception:
                            pass
                else:
...
async def _post_embed_to_thread(thread_id: str, embed: discord.Embed) -> None:
    """Post a rich embed to a Discord thread using the bot client."""
    if not _bot:
        return
    try:
        channel = _bot.get_channel(int(thread_id))
        if channel is None:
            channel = await _bot.fetch_channel(int(thread_id))
        await channel.send(embed=embed)
    except Exception as exc:
        logger.debug(f"[nodes] _post_embed_to_thread failed: {exc}")
...
def _last_seen_str(node: dict) -> str:
    """
    Return a human-readable last-seen string.
    • Local node          → 'Always connected (local)'
    • No last_seen / no stats ever → 'Never connected'
    • Otherwise          → Discord relative timestamp  <t:UNIX:R>  e.g. '2 minutes ago'
    """
    if node.get("type") == "local":
        return "Always connected (local)"
    last = node.get("last_seen")
    if not last or not node.get("stats"):
        return "⚠️ Never connected — agent has not sent a heartbeat yet"
    try:
...
def _connection_status_str(node: dict) -> str:
    """Full status line including connection detail."""
    if node.get("type") == "local":
...
async def maybe_select_node(ctx) -> Optional[str]:
    ...
    • 0 remote nodes online  → auto-selects local immediately (no UI).
    • 1+ remote nodes online → shows a Discord selection UI so the user
                               can choose between local and any remote node.
    ...
    # Only skip the picker when there are no remote nodes at all
...
def register_commands(bot) -> None:
    ...
    async def node_add(interaction: discord.Interaction):
        ...
        # bot.http.token may include a "Bot " prefix — strip it so the
        # displayed token is the raw token the agent needs.
        raw_token = _bot_token() or ""
        if raw_token.startswith("Bot "):
            raw_token = raw_token[4:]
        bot_token_display = raw_token or "<YOUR_BOT_TOKEN>"
    ...
    async def node_status(interaction: discord.Interaction, node_id: str = ""):
        ...
        for nid, n in target.items():
            ...
            s      = n.get("stats", {})
            # colour: green=connected, red=disconnected, grey=never connected
            has_stats = bool(s)
            color = 0x57F287 if online else (0xED4245 if has_stats else 0x747F8D)
            e = _embed(f"🖥️  {n.get('name', nid)}", color)
