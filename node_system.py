...
# ── DNODE token helpers ───────────────────────────────────────────────────────
# DNODE token = "DNODE_" + base64url( JSON({channel_id, reg_token, node_id}) )
# This lets the install.sh decode all info from a single token — no extra args needed.

def _encode_dnode_token(channel_id: str, reg_token: str, node_id: str) -> str:
    payload = json.dumps({
        "c": channel_id,
        "r": reg_token,
        "n": node_id,
...
async def _handle_agent_register(
    ...
    try:
        ...
        logger.debug(f"[nodes] Could not send registration confirmation: {exc}")

    # Notify admin via DM
    if _bot:
        e = _embed("🟢  Node Connected", 0x57F287)
        e.add_field(name="Name",       value=f"`{hostname}`",               inline=True)
...
async def remote_execute(node_id: str, command: str, timeout: int = 120) -> str:
    ...
    # Send job to node via its Discord channel
    ...
    if not sent:
        ...
        raise RuntimeError(f"Could not deliver job to node {node_id} — channel not found.")
            ...
            for node_id, node in list(nodes.items()):
                ...
                if currently_online:
                    ...
                    if _bot:
                        ...
                        e.description = f"**{name}** (`{node_id}`) reconnected."
                        asyncio.create_task(_notify_admin(e))
                else:
                    ...
                    if _bot:
                        ...
                        e.description = (
                            ...
                            f"Restart `node_agent.py` on that machine to reconnect."
                        )
                        asyncio.create_task(_notify_admin(e))
        except Exception as exc:
...
def _stats_line(node: dict) -> str:
    ...
    du   = s.get("disk_used_gb")
    dt   = s.get("disk_total_gb")
    disk = f"{du:.1f}/{dt:.1f} GB" if du is not None and dt else "—"
    return f"CPU `{cpu}%`  RAM `{ram}`  Disk `{disk}`  VPSes `{vps}`"


def _last_seen_str(node: dict) -> str:
    if node.get("type") == "local":
        return "Always connected (local)"
    if node.get("type") == "pending":
        return "⚫ Awaiting first connection"
    last = node.get("last_seen")
    if not last or not node.get("stats"):
        return "⚠️ Never connected — run the node agent to register"
    try:
        dt   = datetime.fromisoformat(last)
        unix = int(dt.replace(tzinfo=timezone.utc).timestamp())
        return f"<t:{unix}:R>  (`{last[:19]} UTC`)"
    except Exception:
        return f"`{last}`"
...
def _connection_status_str(node: dict) -> str:
    if node.get("type") == "local":
        return "🟢 **Connected** (local machine)"
    if node.get("type") == "pending":
    ...
    if not last or not stats:
        return "⚫ **Never connected** — run the node agent to register"
    if online:
        return "🟢 **Connected** — agent is sending heartbeats"
    return "🔴 **Disconnected** — no heartbeat received recently"
...
def _generate_install_sh(dnode_token: str, tunnel_url: str) -> str:
    """
    Generate the install.sh script that will be served at /install.sh.
    The DNODE token encodes channel_id + reg_token + node_id, so the script
...
mkdir -p "$INSTALL_DIR"
mkdir -p /etc/darknodes

# ── Download node_agent.py ──────────────────────────────────────────────────
echo "[2/6] Downloading node_agent.py from $MANAGER_URL..."
...
# ── Register the node ───────────────────────────────────────────────────────
echo "[4/6] Registering node with DarkNodes manager..."
# Decode DNODE token (base64 JSON containing channel_id and reg_token)
DECODED=$(python3 -c "
import base64, json, sys
token = '$DNODE_TOKEN'
if not token.startswith('DNODE_'):
    print('invalid token', file=sys.stderr); sys.exit(1)
b64 = token[6:]
b64 += '=' * (-len(b64) % 4)
data = json.loads(base64.urlsafe_b64decode(b64).decode())
print(data['c'])  # channel_id
print(data['r'])  # reg_token
")
CHANNEL_ID=$(echo "$DECODED" | head -1)
REG_TOKEN=$(echo "$DECODED" | tail -1)

DISCORD_TOKEN="$DISCORD_TOKEN" python3 "$INSTALL_DIR/node_agent.py" \\
  --channel "$CHANNEL_ID" \\
  --reg-token "$REG_TOKEN" \\
  --config "$INSTALL_DIR/node_agent.json" &
AGENT_PID=$!

# Wait up to 70 seconds for registration to complete
WAITED=0
...
import json, sys
try:
  data = json.load(open('$INSTALL_DIR/node_agent.json'))
  sys.exit(0 if data.get('node_id') else 1)
except: sys.exit(1)
" 2>/dev/null; then
...
" 2>/dev/null; then
  echo "ERROR: Registration timed out. Check that the bot is running and DISCORD_TOKEN is correct."
  kill $AGENT_PID 2>/dev/null || true
  exit 1
fi
...
ExecStart=/usr/bin/python3 $INSTALL_DIR/node_agent.py --config $INSTALL_DIR/node_agent.json
...
echo ""
echo "  After a reboot the agent starts automatically."
echo "  Credentials are saved — no re-registration needed."
echo "========================================"
"""
...
async def _start_node_manager_http() -> None:
    """
    Start a minimal aiohttp HTTP server that serves:
...
class _TunnelSetupModal(discord.ui.Modal, title="Cloudflare Tunnel Setup"):
    tunnel_token = discord.ui.TextInput(
        label="Cloudflare Tunnel Token",
        placeholder="Paste your tunnel token from the Cloudflare Zero Trust dashboard",
        required=True,
        max_length=500,
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not self._token:
            ...
            self._error = "Hostname must start with https:// (or http://)"
            await interaction.response.send_message(
                embed=_err("Invalid Hostname", self._error), ephemeral=True
            )
        else:
...
def register_commands(bot_instance) -> None:
    ...
    async def node_tunnel(interaction: discord.Interaction):
        ...
        if tunnel_cfg.get("configured") and tunnel_cfg.get("tunnel_url"):
            ...
            # Live status check
            reachable, reach_msg = await _verify_tunnel(url)

            # cloudflared service status
            svc_ok, svc_out = await _shell("systemctl is-active cloudflared 2>/dev/null || echo inactive")
            ...
            e.add_field(name="Service",       value=f"`{svc_status}`", inline=True)
            e.add_field(
                name="Tunnel Reachable",
                value=("✅ Online" if reachable else "⚠️ Offline / Starting"),
                inline=True,
            )
            e.add_field(name="Health Check",  value=reach_msg,        inline=False)
            e.add_field(
                name="ℹ️  To reconfigure",
                value=(
                    "Run `/node tunnel` with a new token to overwrite the current configuration.\n"
                    "A new modal will open — enter your updated tunnel token and hostname."
                ),
                inline=False,
            )

            # Offer a Reconfigure button
            view = _TunnelReconfigureView()
            await interaction.response.send_message(embed=e, view=view, ephemeral=True)
            await view._event.wait()
            if not view._reconfigure:
                return
            # Fall through to setup flow below (user clicked Reconfigure)
            await interaction.followup.send(
                "Opening tunnel setup form…", ephemeral=True
            )

        # ── Show modal to collect tunnel token + hostname ─────────────────────
        modal = _TunnelSetupModal()
        try:
            await interaction.response.send_modal(modal)
        except discord.errors.InteractionResponded:
            # Interaction was already responded to (reconfigure path uses followup)
            # We need a fresh interaction — ask the user to run the command again
            await interaction.followup.send(
                embed=_err(
                    "Please Run Again",
                    "Discord requires a fresh interaction to open the setup form.\n"
                    "Please run `/node tunnel` again.",
                ),
                ephemeral=True,
            )
            return
        ...
        if modal._error or not modal._token or not modal._host:
            # Error was already sent by the modal's on_submit
            return
    ...
    async def node_add(interaction: discord.Interaction):
        ...
        # Build the DNODE composite token
        dnode_token = _encode_dnode_token(channel_id, token_code, node_id)
        ...
        embed.description = (
            "A private relay channel has been created for this node.\n"
            "Choose your preferred installation method below."
        )

        # ── Method A: one-liner curl ──────────────────────────────────────────
        embed.add_field(
            name="🚀  Method A — One-liner (recommended)",
            value=(
                "Run this on the remote machine as **root** or with **sudo**:\n"
                f"```bash\n{curl_cmd}\n```"
                "The script automatically:\n"
                "• Downloads `node_agent.py`\n"
                "• Registers the node\n"
        ...
        # ── Method B: manual ─────────────────────────────────────────────────
        embed.add_field(
            name="🔧  Method B — Manual setup",
            value=(
                "**Step 1** — Set the bot token on the remote machine:\n"
                "```bash\nexport DISCORD_TOKEN=\"your_bot_token_here\"\n```\n"
                "**Step 2** — Register the node (one-time):\n"
                f"```bash\npython3 node_agent.py \\\n"
                f"  --channel {channel_id} \\\n"
                f"  --reg-token {token_code}\n```\n"
                "**Step 3** — All future restarts (credentials auto-saved):\n"
                "```bash\npython3 node_agent.py\n```"
            ),
            inline=False,
        )
        ...
        embed.add_field(name="⏰  Token Expires",   value=f"<t:{exp_ts}:R>",    inline=True)
        embed.add_field(name="🔗  Node ID",         value=f"`{node_id}`",        inline=True)
        embed.add_field(name="📺  Channel",          value=channel.mention,       inline=True)

        # ── After registration note ───────────────────────────────────────────
        embed.add_field(
            name="✅  After first registration",
            value=(
                "Credentials are **saved automatically** to `node_agent.json`.\n"
                "If the server reboots, the systemd service restarts the agent automatically — "
                "no manual intervention or re-registration needed."
            ),
            inline=False,
        )
        ...
        embed.add_field(
            ...
            value=(
                "• Agent makes **outbound HTTPS** to `discord.com` only\n"
                "• No inbound ports, no public IP, works behind CGNAT/VPN\n"
                "• Token is single-use and expires automatically\n"
                "• The bot receives heartbeats and jobs via Discord channels"
            ),
            inline=False,
        )
    ...
    async def node_remove(interaction: discord.Interaction, node_id: str):
        ...
        if cid:
            e.add_field(name="Channel", value=f"(deleted)", inline=True)
    ...
    async def node_list(interaction: discord.Interaction):
        ...
        for nid, n in nodes.items():
            tag   = "`Local`" if nid == LOCAL_NODE_ID else ("`Pending`" if n.get("type") == "pending" else "`Remote`")
            s     = n.get("stats", {})
            cpu   = f"`{s.get('cpu', '—')}%`"
            u, t  = s.get("ram_used_mb"), s.get("ram_total_mb")
            ram   = f"`{u}/{t} MB`" if u is not None and t else "`—`"
            du, dt = s.get("disk_used_gb"), s.get("disk_total_gb")
            disk  = f"`{du:.1f}/{dt:.1f} GB`" if du is not None and dt else "`—`"
            ...
            embed.add_field(
                ...
                value=(
                    f"**ID:** `{nid}`\n"
                    f"**Status:** {_connection_status_str(n)}{ip_ln}\n"
                    f"**Last Seen:** {_last_seen_str(n)}\n"
                    f"**CPU:** {cpu}  **RAM:** {ram}  **Disk:** {disk}  **VPSes:** {vps_c}"
                ),
                inline=False,
            )
    ...
    @app_commands.describe(node_id="Node ID (from /node list)")
    async def node_info(interaction: discord.Interaction, node_id: str):
        ...
        e         = _embed(f"🖥️  {n.get('name', node_id)}", color)

        e.add_field(name="ID",         value=f"`{node_id}`",               inline=True)
        e.add_field(name="Type",       value=n.get("type", "?").title(),   inline=True)
        e.add_field(name="Hostname",   value=f"`{n.get('hostname','?')}`", inline=True)
        e.add_field(name="Connection", value=_connection_status_str(n),    inline=False)

        if n.get("public_ip"):
            e.add_field(name="Public IP", value=f"`{n['public_ip']}`",    inline=True)

        cid = n.get("channel_id", "")
        if cid and _bot:
            ch = _bot.get_channel(int(cid))
            if ch:
                e.add_field(name="Relay Channel", value=ch.mention,       inline=True)

        e.add_field(name="Created",   value=f"`{n.get('created_at','?')[:19]} UTC`", inline=True)
        e.add_field(name="Last Seen", value=_last_seen_str(n),            inline=False)
        ...
        if s:
            ...
            e.add_field(name="Disk",  value=f"`{du:.1f}/{dt:.1f} GB`" if du is not None and dt else "`—`", inline=True)
            e.add_field(name="VPSes", value=f"`{s.get('running_vps','—')}`", inline=True)

        await interaction.followup.send(embed=e)

    bot_instance.tree.add_command(node_group)
    logger.info("[nodes] /node command group registered (add|list|info|rename|remove|tunnel)")
...
async def startup(bot_instance=None) -> None:
    ...
    logger.info("[nodes] Node system started — relay via Discord channels + HTTP server on 127.0.0.1:{NODE_MANAGER_PORT}")
