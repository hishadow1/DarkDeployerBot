...
async def _heal_node_channel(node_id: str) -> tuple:
    ...
    e = _embed("⚠️  Node Channel Recreated — Reconnect Required", 0xFEE75C)
    e.description = (
        f"The Discord channel for node **{node.get('name', node_id)}** (`{node_id}`) "
        f"was deleted or became unreachable.\n\n"
        f"A replacement channel {new_ch.mention} has been created automatically. "
        f"**Deployments to this node will fail until the agent reconnects.**"
    )
    e.add_field(
        name="🔧  Reconnect (if `darknodes-agent` service is installed)",
        value=(
            f"```bash\n"
    ...
    e.add_field(
        name="ℹ️  Note",
        value=(
            "All existing VPSes and node settings are preserved.\n"
            "You do **not** need to delete and re-add this node."
        ),
        inline=False,
    )
...
async def _start_node_manager_http() -> None:
    ...
    async def handle_api_reconnect(request: web.Request) -> web.Response:
        ...
        if _bot:
            e = _embed("🔄  Node Manually Reconnected", 0x5865F2)
            e.add_field(name="Name",     value=f"`{node.get('name','?')}`",       inline=True)
            e.add_field(name="ID",       value=f"`{node_id}`",                    inline=True)
            e.add_field(name="Hostname", value=f"`{node.get('hostname','?')}`",   inline=True)
            asyncio.create_task(_notify_admin(e))
...
def register_commands(bot_instance) -> None:
    ...
    async def node_tunnel(interaction: discord.Interaction):
        ...
        if tunnel_cfg.get("configured") and tunnel_cfg.get("tunnel_url"):
            ...
            e.add_field(name="Configured At",    value=f"`{set_at} UTC`",                      inline=True)
            e.add_field(name="cloudflared",      value=f"`{svc_status}`",                      inline=True)
            e.add_field(name="Reachable",        value="✅ Online" if reachable else "⚠️ Offline / Starting", inline=True)
            e.add_field(name="Health Check",     value=reach_msg,                              inline=False)
            e.add_field(
                name="ℹ️  Need to reconfigure?",
                value=(
                    "Click **Reconfigure** to go through the setup again with a different "
                    "tunnel token or hostname.  The guide will be shown again."
                ),
                inline=False,
            )
    ...
    async def node_add(interaction: discord.Interaction):
        ...
        embed.description = "Choose your preferred installation method below."
        ...
        embed.add_field(
            ...
            value=(
                "**Step 1** — Download the agent on the remote machine:\n"
                f"```bash\ncurl -fsSL '{tunnel_url_str}/node_agent.py' -o node_agent.py\n```\n"
                "**Step 2** — Register the node (one-time, no Discord token needed):\n"
    ...
    async def node_reconnect(interaction: discord.Interaction, node_id: str):
        ...
        e.description = (
            f"A **{RECONNECT_TOKEN_EXPIRY_MIN}-minute** reconnect token has been generated "
            f"for **{node.get('name', node_id)}**.\n"
            "Run one of the commands below on the remote machine to reconnect manually."
        )
        e.add_field(name="🖥️  Node",    value=f"`{node.get('name', node_id)}` (`{node_id}`)", inline=True)
        e.add_field(name="⏰  Expires", value=f"<t:{exp_ts}:R>",                              inline=True)
        e.add_field(
            name="🚀  Recommended — Stop service, reconnect, restart",
            value=f"```bash\n{service_cmd}\n```",
            inline=False,
        )
        e.add_field(
            name="🔧  Manual (if service is not installed)",
            value=f"```bash\n{reconnect_cmd}\n```",
            inline=False,
        )
        ...
        e.add_field(
            ...
            value=(
                "• Verifies the token with the Node Manager\n"
                "• Restores the node's **existing** identity and credentials\n"
                "• **No new node is created** — all VPSes and settings are preserved\n"
                "• The token is single-use and expires in "
                f"{RECONNECT_TOKEN_EXPIRY_MIN} minutes"
            ),
            inline=False,
        )
        await interaction.response.send_message(embed=e, ephemeral=True)
    ...
    async def node_regenerate_token(interaction: discord.Interaction, node_id: str):
        ...
        e = _embed("🔑  Registration Token Regenerated", 0x57F287)
        e.description = (
            f"A new registration token has been generated for **{node.get('name', node_id)}**.\n"
            f"Previous pending tokens for this node have been invalidated."
        )
        e.add_field(name="🖥️  Node",    value=f"`{node.get('name', node_id)}` (`{node_id}`)", inline=True)
        e.add_field(name="⏰  Expires", value=f"<t:{exp_ts}:R>",                              inline=True)
        if stale:
            e.add_field(name="🗑️  Cleared", value=f"`{len(stale)}` old token(s) invalidated", inline=True)

        e.add_field(
            name="🚀  Method A — One-liner reinstall (recommended)",
            value=(
                "Run as **root** or with **sudo** on the remote machine:\n"
                f"```bash\n{curl_cmd}\n```"
                "The install script removes stale credentials and re-registers the node.\n"
                "**All existing VPSes and settings are preserved.**"
            ),
            inline=False,
        )
        e.add_field(
            name="🔧  Method B — Manual re-registration",
            value=(
                "If the service is already installed, re-register then restart:\n"
                f"```bash\npython3 /opt/darknodes/node_agent.py --dnode-token \"{dnode_token}\"\n"
                f"systemctl restart darknodes-agent\n```"
            ),
            inline=False,
        )
        e.add_field(name="🔑  DNODE Token", value=f"```\n{dnode_token}\n```", inline=False)
        e.add_field(
            name="ℹ️  When to use this",
            value=(
                "Use this command only when:\n"
                "• The node agent was **reinstalled** and lost its `node_agent.json`\n"
                "• Credentials were **accidentally deleted**\n\n"
                "For temporary disconnections or manual recovery, use `/node reconnect` instead."
            ),
            inline=False,
        )
