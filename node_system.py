...
def _embed(title: str, color: int = 0x5865F2) -> discord.Embed:
    ...
    e.set_footer(text=f"{_brand()}  •  Node Manager", icon_url=logo if logo else discord.Embed.Empty)
    return e
...
def register_commands(bot_instance) -> None:
    ...
    async def node_add(interaction: discord.Interaction):
        ...
        embed = _embed("➕  New Node Token", 0x57F287)
        embed.description = (
            f"Run the command below on your remote machine to register **node `{node_id}`**."
        )
        embed.add_field(name="🔑 Node ID",      value=f"`{node_id}`",      inline=True)
        embed.add_field(name="⏰ Expires",       value=f"<t:{exp_ts}:R>",  inline=True)
        embed.add_field(name="☁️ Manager URL",   value=f"`{tunnel_url_str}`", inline=True)

        embed.add_field(
            name="🚀  Recommended — One-liner install",
            value=(
                f"```bash\n{curl_cmd}\n```"
                "-# Downloads agent, registers node, installs systemd service."
            ),
            inline=False,
        )
        embed.add_field(
    ...
    async def node_remove(interaction: discord.Interaction, node_id: str):
        ...
        e.description = f"**{node.get('name', node_id)}** has been unregistered."
    ...
    async def node_rename(interaction: discord.Interaction, node_id: str, name: str = ""):
        ...
        e.add_field(name="🔑 Node ID",   value=f"`{node_id}`",  inline=True)
        e.add_field(name="🏷️ Was",       value=f"`{old}`",      inline=True)
        e.add_field(name="✅ Now",        value=f"`{new_name}`", inline=True)
        await interaction.response.send_message(embed=e)
    ...
    async def node_list(interaction: discord.Interaction):
        ...
        for nid, n in nodes.items():
            ...
            if s:
                ...
                disk_pct = int(du / dt * 100) if dt else 0
                res_line = (
                    f"`CPU {_rbar(cpu,100,8)}` **{int(cpu)}%**  "
                    f"`RAM {_rbar(u,t,8)}` **{ram_pct}%**  "
                    f"`DSK {_rbar(du,dt,8)}` **{disk_pct}%**  "
                    f"🖥️ **{vps_c}** VPS"
                )
            else:
                res_line = "*No stats — agent not connected*"

            status_line = _connection_status_str(n)
            embed.add_field(
                name=f"{dot}  {name}  ·  {tag}",
                value=f"**ID** `{nid}`{ip_str}\n{status_line}\n{res_line}",
                inline=False,
            )
    ...
    async def node_info(interaction: discord.Interaction, node_id: str):
        ...
        e = _embed(f"{_status_dot(n)}  {display_name}  ·  {tag}", color)
        ...
        if s:
            ...
            e.add_field(
                ...
                value=(
                    ...
                    f"🧠 RAM  `{_rbar(u, t, 12)}` **{ram_pct}%**  ({_fmt_ram(u, t)})\n"
            ...
            e.add_field(
                name="📊 Resources",
                value="*Agent has not sent stats yet.*",
                inline=False,
            )
    ...
    async def node_reconnect(interaction: discord.Interaction, node_id: str):
        ...
        e.description = (
            f"Run the command below on **{node.get('name', node_id)}** to restore the connection.\n"
        ...
        e.add_field(
            name="🚀  With systemd service (recommended)",
            value=f"```bash\n{service_cmd}\n```",
            inline=False,
        )
        e.add_field(
    ...
    async def node_regenerate_token(interaction: discord.Interaction, node_id: str):
        ...
        e.add_field(
            name="🚀  Recommended — One-liner reinstall",
            value=(
                f"```bash\n{curl_cmd}\n```"
                "-# Re-registers the node. All VPSes and settings are preserved."
            ),
            inline=False,
        )
        e.add_field(
            name="🔧  Manual — Re-register and restart",
            value=(
                f"```bash\n"
                f"python3 /opt/darknodes/node_agent.py --dnode-token \"{dnode_token}\"\n"
                f"systemctl restart darknodes-agent\n"
