...
def _stats_line(node: dict) -> str:
    ...
    cpu_s  = f"{cpu}%" if s else "—"
    ram_s  = f"{u}/{t} MB"   if u is not None and t else "—"
    disk_s = f"{du:.1f}/{dt:.1f} GB" if du is not None and dt else "—"
    cpu_b  = _rbar(cpu, 100, 8)
    ram_b  = _rbar(u or 0, t or 0, 8)
    dsk_b  = _rbar(du or 0, dt or 0, 8)
    return (
        f"`{cpu_b}` **CPU** {cpu_s}  |  "
        f"`{ram_b}` **RAM** {ram_s}  |  "
        f"`{dsk_b}` **Disk** {disk_s}  |  "
...
def register_commands(bot_instance) -> None:
    ...
    async def node_add(interaction: discord.Interaction):
        ...
        embed.description = (
            f"A new remote node has been prepared with ID `{node_id}`.\n"
            "Run one of the commands below on your remote machine to register it."
        )
        ...
        embed.add_field(
            ...
            value=(
                ...
                f"```bash\n{curl_cmd}\n```"
                "> Downloads the agent, registers this node,\n"
                "> installs a systemd service, and enables auto-start on boot."
            ),
            inline=False,
        )
        ...
        # ── Method B: manual ─────────────────────────────────────────────────
        embed.add_field(
            name="🔧  Method B — Manual (step by step)",
            value=(
                f"**1.** Download the agent:\n"
                f"```bash\ncurl -fsSL '{tunnel_url_str}/node_agent.py' -o node_agent.py\n```"
                f"**2.** Register (one-time):\n"
                f"```bash\npython3 node_agent.py --dnode-token \"{dnode_token}\"\n```"
                f"**3.** All future runs (creds auto-saved):\n"
        ...
        embed.add_field(
            name="ℹ️  After registration",
            value=(
                "Credentials are saved to `node_agent.json` automatically.\n"
                "The systemd service restarts the agent on reboot — no manual action needed."
            ),
            inline=False,
        )
    ...
    async def node_remove(interaction: discord.Interaction, node_id: str):
        ...
        e = _embed("🗑️  Node Removed", 0xED4245)
        e.description = "The node has been unregistered and its channel deleted."
        e.add_field(name="🔑 Node ID",  value=f"`{node_id}`",              inline=True)
        e.add_field(name="🏷️ Name",     value=f"`{node.get('name','?')}`", inline=True)
        e.add_field(name="📢 Channel",  value="Deleted" if cid else "None", inline=True)
    ...
    async def node_rename(interaction: discord.Interaction, node_id: str, name: str = ""):
        ...
        e.description = f"Successfully renamed node `{node_id}`."
        e.add_field(name="🏷️ Old Name", value=f"`{old}`",      inline=True)
    ...
    async def node_list(interaction: discord.Interaction):
        ...
        embed = _embed("🌐  Node List", 0x5865F2)
        embed.description = (
            f"**{total_nodes}** node(s) registered  •  "
            f"🟢 **{online_count}** online  •  "
            f"🔴 **{total_nodes - online_count}** offline"
        )

        for nid, n in nodes.items():
            ntype = n.get("type", "remote")
            if nid == LOCAL_NODE_ID:
                tag_icon, tag_label = "🏠", "Local"
            elif ntype == "pending":
                tag_icon, tag_label = "⏳", "Pending"
            else:
                tag_icon, tag_label = "🌐", "Remote"
            ...
            ip_ln  = f"  •  🌍 `{n['public_ip']}`" if n.get("public_ip") else ""

            cpu_bar  = _rbar(cpu, 100, 8)
            ram_bar  = _rbar(u, t, 8)
            disk_bar = _rbar(du, dt, 8)

            embed.add_field(
                name=f"{_status_dot(n)}  **{n.get('name', nid)}**  {tag_icon} {tag_label}",
                value=(
                    f"🔑 `{nid}`{ip_ln}\n"
                    f"{_connection_status_str(n)}\n"
                    f"⚙️ `{cpu_bar}` **{cpu}%**  🧠 `{ram_bar}` **{u}/{t} MB**  "
                    f"💾 `{disk_bar}` **{du:.1f}/{dt:.1f} GB**  🖥️ VPSes **{vps_c}**"
                ),
                inline=False,
            )
    ...
    async def node_info(interaction: discord.Interaction, node_id: str):
        ...
        e = _embed(f"{_status_dot(n)}  {n.get('name', node_id)}  ·  {tag_str}", color)
        e.description = _connection_status_str(n)

        # ── Identity row ────────────────────────────────────────────────────
        e.add_field(name="🔑 Node ID",   value=f"`{node_id}`",                    inline=True)
        e.add_field(name="🏷️ Hostname",  value=f"`{n.get('hostname','?')}`",      inline=True)
        ...
        if s:
            ...
            e.add_field(
                name="📊 Resource Usage",
                value=(
                    f"⚙️ **CPU**   `{_rbar(cpu,100,12)}` {cpu_pct}%\n"
                    f"🧠 **RAM**   `{_rbar(u,t,12)}` {ram_pct}%  ({u}/{t} MB)\n"
                    f"💾 **Disk**  `{_rbar(du,dt,12)}` {disk_pct}%  ({du:.1f}/{dt:.1f} GB)\n"
                    f"🖥️ **VPSes** {vps} running"
                ),
                inline=False,
            )
    ...
    async def node_reconnect(interaction: discord.Interaction, node_id: str):
        ...
        e.description = (
            ...
            f"Run one of the commands below on **{node.get('name', node_id)}** to reconnect manually."
        )
        e.add_field(name="🏷️ Node",      value=f"`{node.get('name', node_id)}`", inline=True)
        e.add_field(name="🔑 ID",        value=f"`{node_id}`",                   inline=True)
        e.add_field(name="⏰ Expires",   value=f"<t:{exp_ts}:R>",                inline=True)
        e.add_field(
            name="🚀  Recommended — Stop → Reconnect → Restart",
            value=f"```bash\n{service_cmd}\n```",
            inline=False,
        )
        e.add_field(
            name="🔧  Manual (if systemd service is not installed)",
            value=f"```bash\n{reconnect_cmd}\n```",
            inline=False,
        )
        ...
        e.add_field(
            ...
            value=(
                ...
                "> Restores the node's **existing** identity — no new node is created\n"
                "> All VPSes and settings are preserved\n"
                f"> Token is **single-use** and expires in {RECONNECT_TOKEN_EXPIRY_MIN} minutes"
            ),
            inline=False,
        )
        await interaction.response.send_message(embed=e, ephemeral=True)
    ...
    async def node_regenerate_token(interaction: discord.Interaction, node_id: str):
        ...
        e.description = (
            ...
            f"Any previous pending tokens have been invalidated."
        )
        e.add_field(name="🏷️ Node",    value=f"`{node.get('name', node_id)}`",      inline=True)
        e.add_field(name="🔑 ID",      value=f"`{node_id}`",                         inline=True)
        e.add_field(name="⏰ Expires", value=f"<t:{exp_ts}:R>",                      inline=True)
        if stale:
            e.add_field(name="🗑️ Invalidated", value=f"`{len(stale)}` old token(s)", inline=True)
        ...
        e.add_field(
            ...
            value=(
                "Run as **root** or with **sudo** on the remote machine:\n"
                f"```bash\n{curl_cmd}\n```"
                "> Removes stale credentials and re-registers the node.\n"
        ...
        e.add_field(
            ...
            value=(
                "If the service is already installed, re-register and restart:\n"
        ...
        e.add_field(
            ...
            value=(
                "> Use only when the node agent lost its `node_agent.json` or credentials were deleted.\n"
                "> For temporary disconnections, use `/node reconnect` instead."
            ),
            inline=False,
        )
