...
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked \
    ...
    \
    # Housekeeping (apt lists stay in the BuildKit cache mount, not in the layer)
    apt-get clean -y && \
    rm -rf \
       /var/cache/debconf/* \
       /var/log/* \
...
RUN --mount=type=cache,target=/var/cache/apt,sharing=locked \
    --mount=type=cache,target=/var/lib/apt,sharing=locked \
    set -eux; \
    # Add Docker's official GPG key (retries for flaky networks on remote nodes)
...
# ─────────────────────────────────────────────────────────────────────────────
# LAYER 9 — tmate (terminal sharing)
# Strategy: apt-get first (fast, no external CDN), GitHub binary as fallback.
# FATAL — build fails if tmate cannot be installed so the issue is visible
# instead of silently producing containers where the SSH button never works.
# ─────────────────────────────────────────────────────────────────────────────
RUN set -e; \
    arch="$(uname -m)"; \
    # ── Attempt 1: apt package (Ubuntu universe repo, fast + CDN-independent) ─
    if apt-get install -y --no-install-recommends tmate 2>/dev/null && command -v tmate; then \
        echo "tmate installed via apt"; \
    else \
        echo "apt tmate not available — falling back to GitHub release" >&2; \
        case "$arch" in \
            x86_64)  ta="amd64"   ;; \
            aarch64) ta="arm64v8" ;; \
            armv7l)  ta="arm32v7" ;; \
            *) echo "tmate: unsupported arch $arch" >&2; exit 1 ;; \
        esac; \
...
# Pinned GitHub release binary.  FATAL — build fails if sshx cannot be
# installed so the issue is immediately visible during image build.
# ─────────────────────────────────────────────────────────────────────────────
RUN set -e; \
    arch="$(uname -m)"; \
    case "$arch" in \
        ...
        *) echo "sshx: unsupported arch $arch" >&2; exit 1 ;; \
    esac; \
    curl --retry 5 --retry-delay 10 --retry-max-time 180 --connect-timeout 30 -fsSL \
